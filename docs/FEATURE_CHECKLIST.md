...
| Durable operation state | Every force/support/order has an immutable assignment, duty state, engagement mode, materialization state, route progress, recall/return policy, and terminal result. | Partial | Stable operation IDs and broad support/enemy-order/runtime fields exist, but there is no single canonical operation state machine. Add assignment anchor separate from tactical target, Engaged -> Regroup -> Returning -> OnStation transitions, idempotent recall, and save/JIP proof before calling orders reliable. | Highest |
...
| Physical response | Near-player enemy responses spawn, move, fight, and fold back. | Broad Alpha / Needs Soak | QRF/support infantry reapplies native RUN movement and tight formation, reconciles living runtime members, and materializes linked unclaimed response vehicles only inside the player event bubble; eligible off-bubble legacy groups remain abstract, while exact paid QRF realizes through SpawnQueue. Spawned support state now follows the living-member centroid instead of elapsed-time interpolation. ETA is an earliest observation only, and normal arrival or recall exit requires two live samples from distinct elapsed seconds within 75m. Exact QRF handoff enters `support_active`; direct target/exit waypoint chains are replaced transactionally and have a three-attempt consecutive-stall budget reset only by a new-best distance improvement of at least 8m. Terminal cleanup preserves dead bodies, roadblock support keeps its vehicle-safe checkpoint behavior, and recall still refunds eligible surviving FIA HR. Restored pre-repair arrival/exit rows are live-distance revalidated once rather than replaying the two-sample window. Fresh packaged actual movement, arrival, recall, late-physicalization, and contact proof remains open. | Highest |
...
6. Runtime-prove the convoy pre-seat vehicle registration and authority-local
   ...
   and transactional waypoint reissue stays within the three-attempt cap.
