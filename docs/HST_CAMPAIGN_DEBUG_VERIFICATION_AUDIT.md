...
## Current Exact Counterattack Outbound-VIRTUAL Restart Proof

The current stamped source closes one narrow external-process persistence gate
for an exact enemy counterattack that remains outbound and `VIRTUAL`. Foundation
passes at 814 script-symbol references. Guarded Workbench validation compiles
5,832 Game files and 11,828 classes at CRC `92fcd4a4`, reports successful script
validation, exits `0`, records zero hard errors, and leaves every guarded cleanup
counter at zero.

The restart driver launched three fresh engine processes against the exact
loaded build prefix `339b72ec3ed6` and Campaign Schema 70. `prepare` created the
reciprocal order, operation, manifest, batch, member-slot, group, force,
projection, pool, and debit graph. Its semantic source and final digests were
both `046514a9170db409`; exact persisted readback passed and projection-scoped
adapter/PhysicalWar runtime claimants were zero. `recover` restored that graph,
changed startup state once, continued it through the inherited production
counterattack lifecycle, advanced progress exactly, and changed the digest to
`1d2aea419e0a8a32`; continuation and persisted readback were exact and runtime
claimants were again zero. `replay` restored the advanced graph in a third fresh
process, changed startup state once through the expected reconciliation, and was
a semantic no-op: source and final digests remained `1d2aea419e0a8a32`, exact
readback passed, and runtime claimants remained zero. The complete fingerprint
chain was exact and all three process exits were zero.

Containment was part of the result, not an after-the-fact assumption. Owned
process, proof-guard, disposable-profile, generated-log, and external-spill
counters all finished at zero. A separate final census found zero engine
processes and zero restart-proof or Workbench guard roots. Watched roots are
detection-only; cleanup may delete only roots authorized by the exact run nonce
and sentinel. No broad watched or spill root is a deletion target.

This proof uses a guard-authorized, one-use proof subclass of the production
counterattack owner. It replaces only the player-proximity/materialization
answer with a deterministic no-nearby-player result; inherited production
`TickOrder` lifecycle, persistence, continuation, and settlement behavior remain
in control. It therefore proves the outbound-`VIRTUAL` reciprocal graph,
external process restart, first-recovery continuation, second-recovery semantic
idempotence, exact persisted readback, and process/file containment for this
cut.

It does **not** certify live player-proximity or materialization behavior,
`PHYSICAL`/`DEMATERIALIZING` restart cuts, prepared-settlement crash cuts, the
native persistence source, live source/target owner or ownership-revision
continuity, zero ownership-transition claimants, package or live
server/client execution, legacy migration, marker runtime, multiplayer/JIP/
reconnect, performance, or soak. Those remain separate gates. The full Campaign
Debug suite also remains uncertified; the latest full-suite totals below are
preceding historical evidence rather than the result of this focused proof.
...
| Schema-69 exact enemy counterattack | Newly admitted contract-`1` counterattacks use one frozen infantry aggregate, one charged pool, direct virtual travel, deterministic combat, casualty-preserving physical/virtual handoff, canonical ownership, return, and survivor-proportional settlement. Appended `PREPARED` terminal intent enforces prepare -> stage -> refund -> record -> finalize and resumes on restore or a same-session tick. Explicit and deterministically derived claimant IDs reject duplicate or foreign cleanup authority; historical rows remain contract `0`, and invalid current graphs quarantine at `-69`. Current debug sampling preserves initial owner identities and classifies bounded follow-up admissions without hiding missing, duplicate, or mutated rows. | Current source `339b72ec3ed63132e46f3df84540d74d3e938d16`, stamp `7736b42`, passes Foundation at 814 references and guarded Workbench validation at 5,832 files/11,828 classes and CRC `92fcd4a4`, with successful script validation, zero hard errors, and exact cleanup. Its guarded three-process proof preserves prepare digest `046514a9170db409`, advances exactly to `1d2aea419e0a8a32` during fresh-process recovery, and leaves that digest unchanged during fresh-process replay; exact persisted readback, fingerprint chaining, zero adapter/PhysicalWar claimants, all-zero exits, and all-zero process/profile/log/spill cleanup pass. The preceding integrated run remains historical wider-suite evidence at 583 PASS/50 WARN/46 FAIL/7 BLOCKED/1 SKIPPED and 5,537/5,685 proven. This focused result closes only the canonical fallback outbound-`VIRTUAL` cut under the deterministic no-nearby-player seam. Live proximity/materialization, physical/dematerializing and prepared-settlement restart cuts, native persistence selection, endpoint owner/revision and ownership-transition claimant proof, package/live-server behavior, migration and marker runtime, multiplayer/network/JIP/reconnect, performance, and soak remain open. |
...
| Persistence | Baseline typed persistence and seeded smoke roundtrip exist. Sealed Schema 67 deep-copies strategic authority, while the sealed Schema-68 startup correction distinguishes a complete fresh fallback from restored corruption and exact-recovers only the observed generated quarantine. Whole-tree profile movement precedes settings/save consumers and is byte-verified before removal. R26 preserves the seeded in-memory roundtrip exactly: summary/report; 11/11 missions, 22/22 assets, 21/21 runtime entities, 9/9 groups, 10/10 runtime vehicles, and 1/1 field vehicles. It also passes all nine exact-QRF prepared-recovery cuts, repeated reconcile/restore no-ops, fail-closed prepared corruption/tamper checks, stable current-provenance settled-tail quarantine, and mutationless-history compatibility. One guarded external harness passes fresh prepare/recover/replay processes for the exact defensive-QRF canonical-fallback interruption cuts. The current guarded exact-counterattack harness separately passes a canonical fallback outbound-`VIRTUAL` prepare readback, first-restart deterministic continuation, and second-restart semantic no-op with exact fingerprint chaining and zero adapter/PhysicalWar claimants. Later phase smoke/report cases may still WARN after subsequent drift. | Generic `persistence.real_restart` remains correctly BLOCKED because the integrated runner is still in-process. The two external results close only their exact defensive-QRF recovery and exact-counterattack outbound-`VIRTUAL` subgates. Native persistence-source selection, live proximity/materialization, physical/dematerializing and prepared-settlement cuts, package/dedicated authority, affected-save recovery, cadence/backlink continuity, retired-tree conflict handling, semantic migration, local-security/event/vehicle, reconnect, world/physical behavior, and soak remain open. Arbitrary old QRF partial rows remain fail-closed. |
