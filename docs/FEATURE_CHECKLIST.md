...
Workbench validation pass. Foundation reports 751 script-symbol references.
...
in log `logs_2026-07-13_11-20-05`. Its JUnit result at
...
fixtures. This focused result does not execute Full Campaign Debug in `HST_Dev`,
the coordinator isolation/artifact path, or live campaign authority. Package,
restart, dedicated/live-server, and network proof remain open.
...
The current Schema 68/settings 24 commitment-aware planning checkpoint is sealed
under the identity recorded above. It retains the bootstrap/profile/marker
...
marker-integrity cleanup plus commitment-aware planning pass Foundation 751 and
final stamped-tree all-target Workbench validation at CRC `e483e71c`. Campaign
planning also has focused official engine-autotest evidence: one testcase, zero
failures, an empty failed list, and `AllExact=true` for all 17 deterministic
Schema-68 planning fixtures in log `logs_2026-07-13_11-20-05`. Full Campaign
...
Source work has entered Blueprint Phase 9 of 13 through sealed Schema 68, but
this is sequence position, not nine
...
| --- | --- | --- | --- | --- | --- |
| CRI-0 Truth and baseline | Campaign Schema 68/runtime-settings Schema 24 remains the current contract; the commitment-aware source/Workbench checkpoint is sealed as `695caf46ce6b4146e5407711b76d5e0c578d7392`, while `f97b12e` remains the prior packaged failure baseline | Shared production fresh bootstrap, exact-signature poisoned-save recovery/rejection, observational live authority checks, 300-second unchanged-warning throttle, staged verified whole-tree move into `$profile:Partisan`, destructive marker-integrity proof/cleanup, and commitment-aware periodic planning are implemented | Foundation 751 and final stamped-tree all-target Workbench validation pass at 5,815/11,768 and CRC `e483e71c`. Focused official engine autotest `HST_TEST_EnemyPlanningCommitmentAuthority` reports one testcase, zero failures, an empty failed list, and `AllExact=true` across all 17 deterministic Schema-68 planning fixtures. The latest package still reproduced `-67`/`-68` quarantine and 598 warnings; no retired tree existed | No | Execute Full Campaign Debug in `HST_Dev` with coordinator isolation/artifacts and live-authority observation, then package-prove fresh authority, affected-save restart recovery, commitment admission/rerank, single-writer retired-tree removal/conflict archival, protected/player marker behavior, and networking. |
...
| CRI-4 Operation records | Complete in source for nine operation types and eleven explicit family consumers | Schema 66 adds a separately typed local-security patrol operation without changing earlier exact contracts | Schema-66 source proof is wired; packaged runtime pending | No | Local-security graphs quarantine at `-66`, never fall back to disposable police projection, and retain compact terminal operation/manifest evidence. Earlier contract and archive gates remain open. |
...
| CRI-9b Persisted enemy planning | Sealed Schema-68 commitment-aware source/Workbench contract | One production factory supplies the exact three-role/two-planner fresh state; unrelated restore corruption remains fail-closed; recovery requires the exact nonempty preset identity, exact cardinality, non-null rows, empty mutation/order arrays, and the known `-67`/`-68` poison at the current second; periodic scoring filters canonical commitment roots, reranks duplicate patrols in the same decision, freezes before side effects, and revalidates admission before debit; unchanged failures remind at 300 seconds; live proof observes through production exact resolvers | Foundation 751 and final stamped-tree all-target Workbench CRC `e483e71c` pass. Official focused engine autotest `HST_TEST_EnemyPlanningCommitmentAuthority` returns `AllExact=true` across all 17 deterministic Schema-68 planning fixtures; its JUnit result records one testcase, zero failures, and an empty failed list. Full Campaign Debug coordinator execution, its isolated artifact, and live-authority observation remain unexecuted | No | Execute Full Campaign Debug in `HST_Dev`, start a new package with executable pools/planners, restart fresh and affected saves, prove commitment admission/rerank and exact recovery, confirm every near miss remains quarantined, and test dedicated/live-server networking and soak without one-second warning/stutter recurrence. |
...
| Durable operation state | Every force/support/order has an immutable assignment, duty state, engagement mode, materialization state, route progress where applicable, recall/return policy, and terminal result. | Implemented Foundation for nine operation types/eleven family consumers / Needs Runtime Proof | Schema 66 adds a route-less durable local-security assignment whose cyclic physical waypoint chain stays process-local. Its exact roster, materialization, fold, settlement, destruction receipt, and rearm epoch are durable; earlier narrow contracts remain unchanged. Generalized encounters, vehicle/multi-root policy, JIP, historical missions/garrisons, and other families remain open. | Highest |
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Versioned save state | Durable campaign facts survive restarts and schema migration. | Implemented Foundation / Sealed Schema-68 Source/Workbench Boundary | Schema 68 adds durable per-enemy planning authority on top of Schema-67 strategic pools/receipts and preserves the earlier exact restore contracts beneath them. Real serialization/restart remains open. | Highest |
...
| Exact force spawn queue | Each force projection realizes one immutable executable manifest through bounded, retryable, restore-safe work and verified registration. | Implemented Foundation / Needs Runtime Proof | The generic adapter now also serves Schema-66 local security beside two QRFs, player Search-and-Destroy, exact enemy/garrison patrols, and all three assassination-guard contracts. Local security begins held, releases only while its town is active, reconciles exact living slots before fold/persistence, and returns survivors to hold. Schema 52 retains its convoy-specific path. Generic vehicles/assets/multi-root, historical mission/aggregate forces, event-driven casualty subscription, and packaged proof remain open. | Highest |
...
| Full Campaign Debug | One button runs a true runtime certification suite and writes structured artifacts without changing the campaign under test. | Broad Alpha / Sealed Schema-67 And Schema-68 Fixtures Wired / Focused Planning Autotest Passed / Needs Full-Suite And Cross-Domain Runtime Proof | Schema 67 resource assertions and 17 Schema-68 planning fixtures are wired. Official focused engine autotest `HST_TEST_EnemyPlanningCommitmentAuthority` ran the planning authority report under log `logs_2026-07-13_11-20-05`; JUnit UTC `2026-07-13T15:20:12.403Z` records one testcase, zero failures, an empty failed list, and `AllExact=true` across all 17 fixtures. The three sealed commitment-aware production-path assertions cover linked-root collapse with blocking precedence, same-cycle duplicate-patrol rerank, queued and equivalent-ID admission, settled/terminal/rival exclusion, stable multi-reject diagnostics, all-committed zero-cost skip, and post-freeze commitment rejection both before pressure and after pressure was marked but before debit. This focused test did not run Full Campaign Debug in `HST_Dev`, the coordinator's isolated campaign-copy/artifact flow, or live-authority observation. Packaged restart, live server, native entities, rendered marker input, networking, reconnect, JIP, and mandatory process restart remain open. | Highest |
...
1. Execute the sealed Schema-68 commitment-aware planning assertions through
   Campaign Debug: blocked-target fallback, mixed-root precedence, same-cycle
   ...
   marked post-freeze race branches. Package-prove those
