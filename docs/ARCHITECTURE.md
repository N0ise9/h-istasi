...
## Schema 69 / Settings 24 Exact Enemy Counterattack Engine-Proof Checkpoint

The active development tree uses Campaign Schema 69 while runtime settings
remain on Schema 24. A newly admitted enemy counterattack now
belongs to one versioned contract-`1` aggregate spanning the enemy order,
...
| Exact enemy counterattack | Schema 69 gives new contract-`1` counterattacks one reciprocal order/operation/manifest/batch/group aggregate, one charged attack or support pool, direct virtual travel, deterministic combat, physical/virtual casualty continuity, canonical ownership transition, return, and proportional survivor refund. The appended `PREPARED` settlement intent enforces prepare, stage, refund, record, and finalize ordering across restore and same-session retry; derived-ID claimant scans reject ambiguous residue. Historical rows remain contract `0`; malformed current graphs quarantine at `-69` without fabrication, deletion, settlement, refund, or outcome. | The scoped source/Foundation/all-target Workbench/focused-engine checkpoint is sealed. Full Campaign Debug Phase 17, serialization/restart, native projection/combat, package/live-server, migration runtime, marker runtime, network/JIP/reconnect, and soak proof remain open. |
...
| Certification | Schema 69/settings 24 is sealed only for source, Foundation 771, all-target Workbench log `logs_2026-07-13_15-41-50` at CRC `3a8bd64f`, and focused engine log `logs_2026-07-13_15-42-52` with `AllExact=1`. The focused run includes a recoverable base-game VM exception before the HST case completes successfully and is not exception-free. | Execute Full Campaign Debug Phase 17, then package-prove serialization/restart, physical/virtual combat, canonical ownership and one-pool settlement, profile migration, markers, rendered UI, performance, dedicated/live server, multiplayer/networking, reconnect, JIP, and soak behavior. |
