...
| `Scripts/Game/HST/Components/HST_LoadoutEditorComponent.c` | Layout root, named region binding, layout-owned left Back/ESC controls, layout-owned mode tabs, layout-owned storage add-items panel shell/grid, row layout population, preview world, dynamic preview cells, legacy factories | Mixed | Major shell regions, left controls, mode tabs, and the right add-items panel are layout-owned. Dynamic item previews and volume fill are allowed data-driven geometry. Remaining save/settings/candidate panels and left storage rail chrome should be replaced by dedicated layouts/controllers. |
...
- Loadout save/settings/candidate panels and left storage rail chrome that still create substantial panel chrome in script.
...
- Finish replacing remaining loadout save/settings/candidate and left storage rail chrome with dedicated layout widgets.
