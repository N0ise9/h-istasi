...
- Full-campaign debug coverage should explicitly map to the phase plan instead of assuming late smoke helpers cover everything.
  ...
  - Field-vehicle persistence can be tested without perturbing the persistence-smoke expected summary by using a separate debug-prefixed runtime vehicle sentinel after the smoke summary capture. Set it up as `field_vehicle`, capture/restore with `HST_CampaignSaveData`, assert exactly one restored copy with id/prefab/kind/source/position/deleted/detached fields preserved, then remove the live sentinel immediately. Keep physical world respawn and real process restart as separate WARN/manual gaps unless the runner actually restarts or invokes the world restore path with cleanup.
