...
| --- | --- | --- | --- | --- | --- |
| CRI-0 Truth and baseline | Complete | Complete for this checkpoint | Repository inventory, foundation validator, actual-project Workbench Game compile/open, and latest runtime-evidence audit complete | No | Current code and documentation agree on schema 43 and one build-identity source. A native Workbench heap crash was bisected to one oversized force-authority proof and removed by extracting and splitting it; the actual addon now compiles its full Game module, creates the game, and remains responsive through normal project-open startup. The latest gameplay runtime artifact still predates schema 43. Debug profiles fail closed outside the development world and use cloned campaign state, but a fresh runtime proof of completion, cancellation, crash recovery, and restart cleanliness is still required. |
...
| CRI-2 Force manifests | Complete for foundation | Garrison quote/confirm vertical slice | Foundation validator and actual-project Workbench Game compile/open pass | No | Schema 43 persists immutable manifests, expiring actor-owned quotes, typed per-slot spawn results, exact transaction links, and truthful legacy migration. The core faction group catalog records and runtime-validates exact ordered execution-prefab rosters. Visible garrison quantities 1/2/4/6/8/12 now request a server quote; confirmation submits only its ID and charges $50 plus one HR per accepted member all-or-nothing. Deterministic proof covers 1/4/7/12, HR-reservation rollback, and five interrupted-confirmation restore boundaries; that proof now runs through a dedicated service with bounded methods so it no longer crashes Workbench compilation. A fresh isolated runtime run is still required. Paid support, manifest-driven physicalization, terminal spawn-result retention, and accepted settlement/archive compaction remain open. |
...
| Stable authority IDs and receipts | Server mutations use persisted monotonic IDs, bounded receipts, and replay-safe results. | Implemented Foundation / Needs Runtime Proof | Schema 43 persists the allocator, bounded command receipts/events, resource transactions, stable operation/garrison/quote/manifest IDs, and exact purchase-time aggregate links. Concurrent open garrison quotes are capped, but accepted quote/manifest/ledger history is intentionally un-compacted; add archive/tombstone retention before long-campaign certification. Fresh isolated runtime and restart evidence is still required. | Highest |
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Versioned save state | Durable campaign facts survive restarts and schema migration. | Implemented Foundation | Campaign schema 43 additionally deep-copies immutable manifests, quotes, nested roster slots, typed spawn results, stable garrison IDs, and quote/manifest transaction links. Older aggregate forces keep their counts and receive an explicit unverified-migration event instead of invented accounting. Workbench validation passes, but schema-43 process-restart proof is still open. | Highest |
...
| Garrison physicalization | Active zones spawn defenders from garrison and composition services. | Broad Alpha | Current activation does not consume the paid manifest and can choose a different composition. Make ForceRuntime/SpawnQueue consume and register every frozen slot before calling the physical roster exact; then add route/position variety, vehicle plans, and survivor fold-back proof. | Highest |
...
5. Run the isolated schema-43 authority cases and a real save/restart replay test
   proving one request ID produces one training mutation and one committed debit,
   and one garrison quote produces one exact purchase-time strategic increment
   plus two committed debits; physical slot realization remains a separate proof.
6. Extend the schema-43 exact manifest/quote contract from completed visible
   garrison recruitment to one paid support path, then make its spawn queue
   consume and verify every frozen slot.
7. Add bounded settlement/archive tombstones for accepted quote/manifest/ledger
   history and terminal retention for typed spawn results without weakening
   duplicate-confirmation replay.
8. Extend stable operation IDs and typed command results through missions,
   support, garrisons, enemy orders, and physical projections.
9. Resume mission, civilian, undercover, and town-influence depth only after the
   authority and certification foundations can produce trustworthy evidence.
10. Run repeated packaged multiplayer reconnect/restart soak across active
   missions, support, orders, garage, undercover, and terminal campaign states.
11. Tune economy, war level, aggression, support pressure, and mission pacing
