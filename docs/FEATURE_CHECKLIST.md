...
| --- | --- | --- | --- | --- | --- |
| CRI-0 Truth and baseline | Complete | Complete for current source checkpoint | Repository inventory/foundation validator, Workbench source validation, and packaged schema-49 evidence audit complete | No | Current code and documentation agree on campaign schema 54. The packaged schema-49 run proved restored stock HUD/Game Master and exposed marker/dialog/radio/civilian defects repaired later in source. The last stamped checkpoint remains schema 53; schema 54 adds a source/Workbench-only exact purchased-garrison patrol cutover and must not be described as packaged proof. The latest Full Campaign Debug artifact predates schemas 43-54 and is not current certification evidence. |
| CRI-1 Authority foundation | Complete | Prior vertical slices plus schema-54 exact purchased-garrison patrol authority | Schema-54 source/Workbench validation boundary; packaged runtime proof unavailable | No | Only newly issued policy-v2 purchased resistance garrisons gain reciprocal quote/manifest/garrison/operation/route/batch/group authority. Historical policy-v1 purchases, initial/enemy aggregate garrisons, historical patrols, and historical convoys remain on their prior contracts. Owner-change/all-dead/campaign-stop/setup or typed spawn/route-failure settlement records one no-refund receipt; packaged save/restart evidence is absent. |
| CRI-2 Force manifests | Complete for foundation | Garrison quote/confirm, durable SpawnQueue, exact infantry adapter, exact QRF/patrol consumers, and exact three-element mission convoy | Source-validation checkpoints passed; physical and restart proof pending | No | Policy-v2 garrison quotes now freeze one non-self-populating executable root plus an arbitrary exact purchased member roster and submit it unchanged to a held SpawnQueue projection. The generic adapter remains one-root infantry-only; the convoy retains its separate PhysicalWar path. Historical/initial/enemy aggregate garrisons, garrison vehicles/assets, generic multi-root execution, and other consumers remain open. |
| CRI-3 Force runtime | Complete in source for four exact infantry operations, one exact mission convoy, and the crewless mixed-group slice | Exact casualty/reprojection, virtual/physical survivor transfer, patrol fold, convoy-element fold/rematerialization, and mixed-group cleanup | Source-validation checkpoints passed; packaged runtime proof unavailable | No | Schema 54 adds an indefinitely looping local garrison patrol whose held slots remain roster authority and whose adapter validation accepts only durable survivors after first handoff. Exact garrison groups are isolated from legacy PhysicalWar activation/fold/cleanup. Real native patrol movement, casualties, fold/rematerialization, three-vehicle behavior, networking, and process restart remain unproven in a package. |
| CRI-4 Operation records | Complete for exact player/enemy infantry-QRF, enemy-patrol, purchased-garrison-patrol, and narrow mission-convoy kernels | Five explicit version-1 operation types with immutable assignment, typed materialization/route/position/settlement state, durable roster linkage, restore normalization, and fail-closed legacy isolation | Schema-54 source/Workbench boundary; focused behavior has not run in a packaged runtime | No | Schema 54 adds `HST_OPERATION_TYPE_GARRISON_PATROL` only for new policy-v2 resistance purchases: held virtual local loop, survivor-only rematerialization, no-refund owner-change/all-dead/campaign-stop/setup or typed spawn/route-failure settlement, marker/UI projection, and `-54` quarantine. Historical policy-v1/initial/enemy aggregate garrisons stay legacy. Schema 53 enemy-patrol and earlier exact consumers retain their existing source boundaries. |
...
| Stable authority IDs and receipts | Server mutations use persisted monotonic IDs, bounded receipts, and replay-safe results. | Implemented Foundation / Needs Runtime Proof | Schema 54 retains prior authority and adds reciprocal policy-v2 garrison quote/manifest/garrison/operation/route/batch/group identity plus deterministic no-refund terminal settlement. Historical aggregates and other legacy transactions still need equivalent boundaries; isolated runtime/restart evidence is required. | Highest |
| Durable operation state | Every force/support/order has an immutable assignment, duty state, engagement mode, materialization state, route progress, recall/return policy, and terminal result. | Implemented Foundation for five exact operation consumers / Needs Runtime Proof | The earlier exact consumers retain their schema-50 through schema-53 behavior. Schema 54 adds an exact purchased-garrison operation that remains on station and advances an infinite local loop through virtual/physical transfer until owner change, all-dead, campaign stop/setup, or typed spawn/route failure. Off-screen combat, generalized contact, terrain/ammunition, vehicles/multi-root, JIP, historical garrisons, and other families remain open. | Highest |
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Versioned save state | Durable campaign facts survive restarts and schema migration. | Implemented Foundation | Campaign schema 54 adds exact purchased-garrison graph/cursor/casualty/settlement validation. Pre-54 policy-v1 purchases and initial/enemy aggregates stay legacy with no invented exact authority. Coherent current physical-shaped rows normalize to held survivor rosters; malformed rows use `-54` quarantine with no refund or legacy conversion. Physical/dematerializing exact garrison patrols join the pre-capture binding/live-position reconciliation boundary. Actual process restart, rematerialization, marker/JIP projection, and settlement replay still need packaged proof. | Highest |
...
| Generated routes | Convoys, QRFs, patrols, roadblocks, and mission movement use route-aware paths. | Broad Alpha / Needs Runtime Proof | The earlier exact cursors retain their behavior. Schema 54 reuses the generated-route cursor for a new policy-v2 purchased-garrison's infinite local loop, preserving waypoint/lap/leg state across fold/restore. Historical policy-v1, initial/enemy aggregate and vehicle garrisons, historical patrols/convoys, counterattacks, roadblocks, and non-convoy missions remain on prior paths. | Highest |
| Physical activation bubble | Near-player zones physicalize and off-screen forces stay abstract unless an active objective requires runtime entities. | Broad Alpha | Five canonical operation consumers now exist. The schema-54 purchased-garrison patrol advances virtually, materializes only living slots inside the bubble, and folds the same survivors outside it. PhysicalWar isolates its exact groups from legacy garrison activation. Historical/initial/enemy aggregates and garrison vehicles retain the older path; packaged native movement/fold remains open. | Highest |
...
| Exact force spawn queue | Each force projection realizes one immutable executable manifest through bounded, retryable, restore-safe work and verified registration. | Implemented Foundation / Needs Runtime Proof | The generic adapter now serves two QRFs, the exact enemy patrol, and the exact purchased-garrison patrol with one root plus exact members. Its current-count check uses durable living slots after handoff so partial survivor rematerialization remains valid. Schema 52 retains its convoy-specific path. Generic vehicles/assets/multi-root, historical aggregates, event-driven casualty subscription, and packaged proof remain open. | Highest |
...
| Map markers | HQ, zones, missions, support, QRFs, orders, garrison patrols, and active deliveries publish linked markers with cleanup proof. | Broad Alpha / Needs Runtime Proof | Schema 54 adds one roster-authoritative purchased-garrison patrol marker at virtual/live position with location, current owner, role, and survivor count, removed after terminal cleanup. Earlier radio/convoy/enemy-patrol fixes remain source-only. Republish and prove marker rendering, cursor/cleanup, reconnect/JIP, and radio destroy/rebuild behavior. | Highest |
...
| Full Campaign Debug | One button runs a true runtime certification suite and writes structured artifacts without changing the campaign under test. | Broad Alpha / Needs Runtime Proof | The latest full run produced 663 cases: 367 PASS, 61 WARN, 218 FAIL, and 17 BLOCKED; it predates schemas 43-54 and contaminated its save. The nine schema-54 `garrison_patrol.*` admission/replay/roster/route/projection/settlement/restore/corruption/marker assertions and earlier fixtures remain source evidence. Native movement/casualty/fold, marker/UI projection, and mandatory process restart still need packaged proof. | Highest |
...
1. Publish the next stamped schema-54 build and run a focused client/server feedback pass. Confirm that
...
8. Execute the isolated campaign-debug suite in `HST_Dev`, prove completion,
   cancellation, interrupted-run recovery, and unchanged live persistence, then
   replace the historical contaminated artifact.
9. Add the authoritative snapshot/delta/JIP projection protocol and prove host,
   client, reconnect, and late-join equality with revisions and tombstones.
10. Introduce one idempotent ownership-transition service before adding virtual
   capture outcomes or wider encounter simulation.
11. After the schema-54 checkpoint, expand exact manifests and operation
    projection to generic vehicles/assets, historical/initial/enemy aggregate
    garrisons only through an explicit migration policy, remaining supports/
    orders, and non-convoy mission forces one consumer at a time.
12. Run repeated packaged multiplayer restart/performance soak, then tune economy,
