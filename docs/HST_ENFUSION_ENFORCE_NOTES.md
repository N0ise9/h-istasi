...
- Long-running campaign smoke/debug actions should run as a server-side sequencer, not as one huge UI command RPC.
  ...
  - Runtime settings schema 20 clarifies arsenal and vehicle loot filtering. The user-facing key is `lootSkipUnlockedItems` / `vehicleLootSkipUnlockedItems`; the old `lootOnlyLockedItems` / `vehicleLootOnlyLockedItems` keys are deprecated aliases. `true` means skip items that are already unlimited in the arsenal, which is the normal default loot flow. Explosive and guided-launcher unlocks default to enabled.
