...
- Current frozen source checkpoint
  ...
  sanity only. All five formal Workbench source-validation targets also pass at
  5,849 files, 12,022 classes, common CRC `0b872024`, zero hard errors, exact
...
- Immediately prior rejected source checkpoint
  ...
  Source, resource-database, settings, and executable identities remained
...
- Immediately prior frozen source checkpoint
  ...
  87/87 counted conditions, 18 ordered zero-delta state rows, zero final
  orphans, zero unapproved diagnostics, and zero residual engine processes.
  Its tracked summary SHA-256 is
  ...
  selected runtime handles and exact-empty selected-town composition, but
  rejected the global core runtime registry because a runtime group row had an
  orphan, duplicate, or dead pointer. Fatal containment deferred complete
  artifact publication. Source, resource database, settings, and executable
  identities remained stable; no owned or unclaimed engine process remained and
  cleanup errors were zero. The portable failed summary SHA-256 is
...
- Extracted PowerShell fixture helpers retain their original scope-qualified
  dependencies. If a reused assertion increments a script-scoped counter, the
  importing script must initialize that counter in its own script scope before
...
- The release-surface publisher self-test passes 74 checks, including 73
  negatives and two guarded receipts, and independently verifies the structured
  candidate-mount attestation. The retention publisher self-test passes 73/73.
  The ledger consumer invokes both exact
  Git-bound verifier scripts and passes 3 valid/optional plus 53 adversarial
...
- Parse mount evidence as structured timestamped `ENGINE : gproj:` records, not
  ...
  exactly two candidate records, two exact paths, one packed row, and zero
  invalid modes. Ignore core/data project rows and command-line echoes. For a
  focused raw run, independently derive the nonce and require the relative
  suffix `PartisanFocusedAutotest/<nonce>/candidate-addons/Partisan/addon.gproj`.
  For a release-surface result, retain the same facts in one nested structured
  candidate-mount attestation rather than ambiguous scalar substring counts.
- Focused counterattack leaf
  ...
  rejected forensic evidence even though its raw JUnit was 14/0/0/0, all 14
  success markers and the exact stamped build banner were present, only two
...
- CRI-101 accepts the fresh encoding-stable pair under clean harness
  ...
  Retail and diagnostic mount attestations are each exact at 2/2/1/0; retail
  retains the exact approved 6-raw/2-event cluster, diagnostic is exact at 0
  raw/0 events, and unapproved diagnostics are zero. Retention leaf
  `20260722T115003Z-4e159a24384c` retains 251 files; its tracked index/ready
  SHA-256 values are
  ...
  It covers five diagnostic plus five standard contexts and retains
  `standardSaveRestorationCertified: false` with `certificationClaim: none`. Both
...
- The historical ordered `history[2]` `rejected-after-full-profile` candidate is
  ...
  HEAD `4f8d7e2d7a39896737fd6754060523bf852c5fa8` accepted run
  `seed1985_t0_p1_u1784445266` as the unchanged package's corrected
  `force_authority` canary. Its 11 cases were 9 PASS/1 WARN/0 FAIL/1 BLOCKED/0
  SKIPPED; focused proof was 35/35 and 87/87; state restoration was 18/0; all
...
reconciliation or spawn-adapter mutation. `MATERIALIZING` remains covered by the
existing exact-infantry checkpoint deferral. A failed campaign-debug isolated
capture must return `null`; returning the previous isolated save would turn a
rejected current checkpoint into a false success.

`PHYSICAL`/`LIVE` position capture is a two-phase transaction:

1. Read-only preflight resolves the family-owned reciprocal graph, requires one
   successful non-held batch, durable living slots, exact adapter/PhysicalWar
   root and member bindings, and a nonzero centroid sampled from living native
   entities. No durable group or operation position changes here.
2. After roster reconciliation and every unrelated physical validator succeeds,
   apply the sampled position to the group and call the family-owned production
   position transition. If any apply fails, roll back all previously applied
   group positions and each operation's strategic position, revision, and last-
   progress second.

Keep live sampling independent of durable evidence. The physical proof first
samples a native centroid, deliberately corrupts the durable group and operation
positions, invokes production persistence, then resolves the native centroid
again. The pre-capture oracle, post-capture native sample, group position,
operation position, serialized readback, and proof carrier must agree. Sampling
only after the durable position is refreshed would make the test circular.

Restore normalization for a successful physical response is process-free. The
batch clears its native group ID, becomes pending and strategically held,
increments reprojection and attempt/lifecycle generations, clears slot entity,
vehicle, native-group, and verification evidence, requeues every living slot,
and preserves confirmed casualty tombstones. The active group clears spawned,
spawn-attempted, runtime-entity, spawned-agent, assigned-waypoint, and stale
failure-reason fields. Its current and source positions align with the operation
strategic cursor, and its current, survivor, last-seen, and durable roster counts
come from strategically living slots. Defensive-QRF runtime reconciliation
repeats those resets defensively; counterattack and garrison-rebuild save
normalizers own the equivalent fold.

The early dematerialization rejection is stricter than older Schema-70 behavior,
but it is not a schema change. A valid already-written Schema-70
`DEMATERIALIZING` counterattack remains readable and normalizes once to held
...
exercise interrupted raw state only on migration-free clones. Production
capture must return `null` for the materializing and dematerializing clones and
must leave both raw and canonical fingerprints unchanged. The physical cut
frame-drives native root/member creation and handoff, captures the independent
live-position oracle, normalizes to one held reprojection, performs bounded
runtime cleanup, and then recovers 75 meters in a new process. Replay changes no
...
owned engine/crash processes, guards, disposable profiles, generated logs,
external spill, temporary roots, mutexes, adapter/PhysicalWar claimants, and
cleanup errors. Watched roots remain detection-only; deletion requires an exact
run nonce and sentinel.
...
- A defensive QRF owns a different strategic receipt shape from a
  counterattack. Require positive support cost, nonnegative attack cost,
  deterministic debit/refund IDs, exactly one claimant for each role, complete
  faction/source/order/operation/manifest/zone backlinks, exact dual-pool
  deltas, debit-before-refund chronology, empty contribution hashes, and the
  shared mutation-shape check. A support-only QRF has zero attack cost/refund;
...
- Proof telemetry must print the real enum/status/reason values. Do not format
  ...
  category, not one complete English failure sentence that can change when a
  more precise validator takes ownership.
...
runs isolated and removed those cascades. Exact-tree R10 keeps Phase 18 at five
of five PASS, restores Phase 20 clock 560 -> 595 -> 560 with an unchanged enemy-
strategic fingerprint, and finishes Phase 22 at four PASS, three physical-
movement WARN, and zero FAIL. R21 contains no local-security materialization
deferral, preserves the exact seeded roundtrip, and restores typed cleanup plus
the open-order leak snapshot to 0 -> 0.

R23 remains the dated exact-QRF defect reproduction: one tracked settlement
failure left one tracked row, two runtime claimants, and total open orders 0 ->
2 despite an exact-zero final state diff. R24
`seed1985_t0_p1_u1784059798`, build `6303e5817a924091258c9cf0dbccdd2e0731c1e3`,
ran 688 cases at 578 PASS/50 WARN/53 FAIL/7 BLOCKED and proved 5,522/5,682
required assertions. It restored typed cleanup, the 0 -> 0 leak snapshot,
Phase-24 core results, and exact-zero state while isolating the remaining QRF
failure to a counterattack one-pool validator plus brittle proof telemetry.

R25b `seed1985_t0_p1_u1784063032`, build
`434b73a16ae92911896fdec095af6bce88168916`, runs the same 688 cases at 577
PASS/51 WARN/53 FAIL/7 BLOCKED and proves 5,523/5,681 required assertions. Both
`enemy_qrf.settlement` and `enemy_qrf.persistence` pass. Typed order cleanup is
settled 0/failures 0/open 0/runtime 0, the open-order leak is 0 -> 0, seeded
...
from both the passing QRF assertions and whole-suite certification.
...
repeated one-second warnings. That sealed source corrects fresh fallback
ordering, limits recovery to the complete known generated signature, applies a
300-second unchanged-warning reminder, observes live enemy authority through
production exact resolvers, moves the whole retired profile tree through
verified staging/conflict archival, and wires a destructive campaign-marker
self-heal/player-marker isolation probe with final repair/cleanup. Its stamped-
tree all-target Workbench log `logs_2026-07-13_09-20-51` compiled 5,815 Game
files/11,768 classes at CRC `0544aa1d`; all five targets validated and zero
Workbench processes survived cleanup. Campaign Debug, package execution,
packaged restart/migration, dedicated-server, multiplayer, and soak evidence
remain open.

The earlier sealed enemy-planning authority checkpoint is Campaign Schema
...
An earlier sealed source/Workbench checkpoint is Campaign Schema 66 while runtime settings
...
- The proof-fixture correction is sealed in historical ordered `history[2]`
  ...
  is `passed-noncertifying`. Clean harness HEAD
  `4f8d7e2d7a39896737fd6754060523bf852c5fa8` then accepted the unchanged
