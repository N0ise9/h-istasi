...
contracts. The current Phase-17/Phase-24 exact-counterattack checkpoint is
source `393733cc165b96ec494c72f96741cf993d400ebd`, UTC
`2026-07-15T10:46:59Z`, label
`schema70-settings24-native-counterattack-proof-ordering`, with stamp commit
`a8210af`. It is schema-neutral and settings-neutral: it adds no persisted
field, enum ordinal, operation contract, or migration rule.

### Exact Counterattack Native-Projection and Casualty-Continuity Boundary
...
The current source extends that production-owned cycle with one real engine
...
and guarded external cleanup was exact. The current proof-ordering checkpoint
removes that artificial post-handoff yield while preserving the asynchronous
death, cleanup, and re-entry yields.

R31 `seed1985_t0_p1_u1784117364` completes that guarded runtime proof on build
...
| Exact enemy counterattack | Schema 69 gives new contract-`1` counterattacks one reciprocal order/operation/manifest/batch/group aggregate, one charged attack or support pool, direct virtual travel, deterministic combat, physical/virtual casualty continuity, canonical ownership transition, return, and proportional survivor refund. The appended `PREPARED` settlement intent enforces prepare, stage, refund, record, and finalize ordering across restore and same-session retry; derived-ID claimant scans reject ambiguous residue. The proof drives one focal order through production-owned `VIRTUAL` -> `MATERIALIZING` -> `PHYSICAL` -> `VIRTUAL`, then through one real engine death, adapter-owned slot retirement, N-1 fold, N-1 re-entry, and final N-1 fold. Successful handoff, `PHYSICAL` confirmation, and the immediately controlled fold or kill stay in one invocation; asynchronous death, cleanup, and re-entry still yield. Exact liveness requires controller-aware and stock damage-state agreement, while binding diagnostics distinguish incomplete ownership from a complete nonliving binding. Generic survivor sampling excludes the active exact aggregate so it cannot compete with adapter casualty authority. Historical rows remain contract `0`; malformed Schema-69 graphs quarantine at `-69` without fabrication, deletion, settlement, refund, or outcome. | R31 completes 687 cases at 577 PASS/50 WARN/52 FAIL/7 BLOCKED/1 SKIPPED and 5,528/5,685 proven. All 11 Phase-17 projection/casualty assertions and both Phase-24 owner assertions pass; N=9 becomes N-1=8, all three physical confirmations settle at observation 1/4, and death settles at observation 1/4. Phase 24 classifies 16/16 owners with two open exact projections. All 18 state deltas are zero, with zero script/HST/crash errors and exact external cleanup. This runtime-proves the corrected same-invocation ordering, while the overall suite remains uncertified. Arrival/combat/capture/return, serialization/restart, package/live-server, migration runtime, markers, network/JIP/reconnect, and soak behavior remain open. |
...
| Campaign Debug isolation | The runner deep-clones campaign state, suspends normal persistence, and restores the live state. Bounded probes additionally capture/restore the shared clock and enemy-strategic fingerprint; the coordinator holds ambient commander cadence while the clone is active. It also holds ambient local-security progression whenever the matching force-spawn worker is held, preventing a debug-only release into MATERIALIZING with no process owner. The native counterattack probe captures/restores adapter process state, keeps successful handoff -> `PHYSICAL` -> controlled fold/kill in one coordinator invocation, yields for asynchronous death/cleanup/re-entry, uses typed cleanup before fail-closed dual-key focal cleanup, explicitly retires orphan entities, and treats a missing tracked exact order as fatal: record failure, stop, restore, save artifacts, and skip later telemetry. | R31 passes all 11 Phase-17 targets and both Phase-24 owner assertions, with initial/casualty/survivor physical settlement and casualty death each observed at 1/4. It classifies 16/16 Phase-24 owners with two open exact projections and restores all 18 tracked state deltas to zero. This proves the corrected proof ordering in the guarded runtime scope. R26 and R28b remain historical positive boundaries for exact QRF and the earlier exact-counterattack casualty cycle respectively. World entities, players, delayed callbacks, caches, and real process restart remain outside the clone boundary; `world_scope` stays BLOCKED. |
...
| Certification | Schema 70/settings 24 remains the current persisted contract. Current implementation `393733cc165b96ec494c72f96741cf993d400ebd`, stamp `a8210af`, UTC `2026-07-15T10:46:59Z`, label `schema70-settings24-native-counterattack-proof-ordering`, adds no schema/settings migration. | The final stamped tree passes Foundation at 808 references. Workbench Game Module validation loads 5,830 files/11,822 classes/47,077K at CRC `b789ee05`, reports `Script validation successful`, exits `0`, records zero script/HST/hard-failure signals, and leaves session/owned-process/default-log/spill cleanup at zero. R31 proves the scoped native projection/casualty ordering with all 11 Phase-17 and both Phase-24 owner assertions passing, an exact-zero 18-row state delta, zero runtime error signals, and exact external cleanup. The full suite remains uncertified at 577 PASS/50 WARN/52 FAIL/7 BLOCKED/1 SKIPPED and 5,528/5,685 required assertions proven. Historical exact-QRF focused/external-restart evidence remains limited to that earlier checkpoint. Native persistence-source selection, world-scope, all-five Workbench, package, arrival/combat/capture/return, markers, rendered UI, performance, dedicated/live server and multiplayer/networking, reconnect/JIP, and soak gates remain open. |
