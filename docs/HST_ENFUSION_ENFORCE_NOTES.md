...
- Player-requested support cooldowns can poison later support smoke steps.
  ...
  - Phase 18/19 smoke assertions should verify debug-prefixed enemy order/support records, expected types, target zones/positions, resource cost fields, and resolve/ETA behavior. Stage 3 QRF/search support now samples one route advance and controlled ETA arrival; keep Phase 19 enemy/support movement, repeated samples, and contact behavior as WARN/separate probes until those paths are sampled over time.
  ...
  - Phase 22 HQ/Defend Petros smoke should assert the state chain, not only the command text: seeded HQ knowledge/threat reasons, a debug-prefixed Petros attack order, a debug-prefixed dynamic defense mission/objective/task, active defense markers, linked support request evidence, one routed attacker-group distance decrease, admin-success resolution, Petros death/runtime-object clearing, and debug recovery. Prefix the defense mission before objective/task creation so cleanup can remove the whole record chain; retag a later physical support request before it spawns a group. Keep multi-wave/contact/arrival/defense pressure as WARN until those transitions are sampled over time.
