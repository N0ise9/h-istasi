...
for the Blueprint Phase 8 ambient-runtime slice. It adds capped global civilian
actor/traffic allocation, leased fair rotation, a four-root scheduler,
asynchronous behavior-ready admission, immutable per-zone/kind projection
slots, bounded slot-specific recovery/recycling, per-frame player-first claim
observation, and a fail-closed
transient-versus-claimed vehicle persistence boundary. The checkpoint identifies
...
validation pass;
deterministic proof services are compiled and wired but have not executed in
Campaign Debug. This is not native runtime certification. Packaged runtime, real
brief enter/exit, autosave/restart, promoted-root destruction, new-campaign
reset, rendered UI, Campaign Debug Phase 20 production-path execution, a ten-
town/ten-minute performance and churn soak, stutter measurement, and multiplayer
...
The current development save contract is sealed Schema 64. One canonical
...
| --- | --- | --- | --- | --- | --- |
| CRI-0 Truth and baseline | Campaign Schema 64/runtime-settings Schema 24 is the current sealed source/Workbench checkpoint; the Schema-23 canonical-town checkpoint precedes it. The Blueprint Phase 8 ambient slice adds no Campaign Schema 65 | The current sealed identity is implementation `6afadc7c13681b78171939a740862e52328beffd`, UTC `2026-07-12T15:57:55Z`, label `schema64-settings24-ambient-runtime-authority`. It adds ambient budgeting/lifecycle/save-boundary work without changing the campaign-state schema | Final sealed-tree evidence is Foundation at 711 references and normal/all-five-configuration Workbench checks at 5,799 files/11,718 classes with CRC `bb083672`, explicit validation success, zero HST script errors, and zero surviving Workbench processes. Campaign Debug and packaged results remain open | No | Sealing or publishing this source/Workbench identity does not certify it. Package/native/save-restart/rendered-UI/ten-town soak/setup/network/reconnect/JIP gates remain independently open. |
...
| CRI-8 Civilian runtime and political consequences | Ambient foundation designed; consequence layer incomplete | Sealed Settings Schema 24 source adds fair global population budgets, behavior-ready pedestrian/traffic transactions, movement recovery/recycling, owner-aware static refresh, and the transient-versus-claimed vehicle save boundary. Existing aid and ownership/security-pressure source paths remain separate from the unfinished consequence work. | Final sealed-tree source/Workbench evidence is Foundation at 711 references and normal/all-five Workbench checks at 5,799 files/11,718 classes with CRC `bb083672`; deterministic proofs are compiled but unexecuted | No | Native-prove the ten-town soak, lifecycle, capture/reset/restoration boundary, and two-nearby-same-prefab no-collapse case. Aid and ownership/security-pressure paths need runtime proof; automatic casualty, theft, nearby-combat influence, panic/recovery, and deeper local-security behavior remain open. |
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Versioned save state | Durable campaign facts survive restarts and schema migration. | Implemented Foundation / Sealed Schema-64 Source Boundary | Schema 64 adds canonical town-influence records and exact event before/after basis-point/revision evidence. Pre-64 migration reconciles legacy zone signed support and civilian pairs once, preserves population without resurrecting destroyed towns, and emits one bounded conflict fact when needed. Current malformed, duplicate, missing, orphaned, or inconsistent authority quarantines at `-64`; events are validated, not replayed. Schema 63 combat heat and Schema 62 ownership receipts remain beneath it. Foundation and Workbench compile/validation pass; Campaign Debug, real serialization, and save/restart proof remain open. | Highest |
| Runtime settings migration | Generated profile settings migrate forward without keeping obsolete setup knobs. | Implemented Foundation / Sealed Settings Schema 24 Source/Workbench | The `23 -> 24` migration adds normalized global civilian actor and traffic budgets, per-player additions, war-level budget penalty, runtime health/startup/stuck/recovery/backoff controls, and preserves explicit older values. Campaign persistence remains Schema 64. Generated profile rewrite and packaged migration proof are pending. | Keep |
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Town support ledger | Events change support, reputation, heat, security, and population with explainable history. | Sealed Canonical Schema-64 Source/Workbench / Needs Runtime Proof | `HST_TownInfluenceRecord` is the sole political support/population truth. Typed events preserve requested/effective faction deltas, population basis, before/after support, revision, source, duration, and replay identity. Radio/security/mission/convoy/order callers route through the service. Mutation-time counters and `next expiry` avoid the former all-towns-by-all-events scan each second. Execute the fixtures and prove expiry, idempotency, save/restart, and bounded history at runtime. | Highest |
...
| Civilian runtime population | True towns have a configurable daytime/low-heat driven-traffic target, default five, when town population and global budgets permit; the target is not a per-town guarantee. Minor localities retain their smaller policy. Logical population remains canonical while disposable physical actors rotate fairly. | Implemented Foundation in sealed Settings Schema 24 source/Workbench / Needs Native Runtime Proof | Global actor/traffic caps use 120-second-or-longer leases and deterministic fair rotation. Each health update begins at most four root-spawn transactions, including first-pass static ambience; the scoped Campaign Debug Phase 20 helper selects its town from that same production global plan and transaction-start cap. Pedestrians require an exact living CIV one-member group and active wander waypoint before admission; traffic requires the exact pilot, usable vehicle, running engine, and active route. Immutable per-zone/kind slots drive distinct deterministic recovery routes. Owner/policy-aware static military refresh and render-bubble cleanup are wired. Player-first observation avoids a full ambient-root occupancy scan before persistence every server frame. Every `HST_PersistenceService` capture/checkpoint path repeats it behind a fail-closed reconciliation barrier, and new-campaign reset performs its own reconciliation. Only live roots with live controlled occupants promote; one tracker covers promotions, restored/adopted field vehicles, and garage redeploys for transform/destruction/cargo snapshots. Saved durable IDs remain stable across restart; restore precedes first-frame claims, exact forward/reverse bindings own durable targeting, and garage redeploy tracks a fresh campaign ID before commit with exact rollback. Reset may retain only occupied live tracked `loot_vehicle`, `field_vehicle`, or `garage_redeploy` roots, normalizes retained rows to `field_vehicle`, copies their vehicle/cargo rows before state replacement, and deletes every other bound root once. Session topology does not dirty the save; unclaimed ambient rows/cargo are filtered, while claimed field vehicles and legacy detached claims persist. Frame-frequency horn clearing walks direct traffic records rather than helper-by-root nested lookup. Pure proof services are compiled/wired but have not executed in Campaign Debug. Run Campaign Debug, packaged native behavior, native brief enter/exit, autosave/restart, destruction, reset, two-nearby-same-prefab no-collapse restoration, Campaign Debug Phase 20 execution, and a ten-town/ten-minute churn/stutter/performance soak. Aid and ownership/security-pressure paths exist but need runtime proof; add automatic casualty, theft, nearby-combat influence, panic/recovery, and deeper local-security behavior. | High |
...
| Performance soak | Physical/abstract transitions do not leave stuck groups, duplicate vehicles, missing markers, or periodic frame stalls. | Needs Soak / Additional Sealed Schema-64 Repair | The latest user run showed a visible once-per-second stutter. Earlier repairs remain; Schema 64 additionally maintains town influence aggregates incrementally, scans history only when a town's recorded expiry is due, and throttles verbose changed active-group survivor/count logs to 30 seconds. Foundation and Workbench checks pass, but neither is runtime profiling evidence. Publish and profile only the exact sealed Settings-24 identity, then measure freezes per minute before calling this fixed. | Highest |

## Highest-Impact Next Tasks

1. Publish only the exact sealed Campaign Schema-64/runtime-settings Schema-24
   identity `schema64-settings24-ambient-runtime-authority` and require that same
   `HST_BuildInfo`
...
3. Runtime-prove civilian projection across at least ten simultaneously eligible
   ...
   retention/normalization, and no-duplication evidence.
