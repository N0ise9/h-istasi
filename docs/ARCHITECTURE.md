...
## Current Counterattack Outbound-VIRTUAL Restart Boundary
...
`replay` in three fresh engine processes. It admits one canonical fallback
outbound-`VIRTUAL` reciprocal order/operation/manifest/batch/group graph,
persists it, and requires exact readback before each later stage. `prepare`
preserved digest `046514a9170db409`; `recover` restored that digest and advanced
the production counterattack continuation exactly to `1d2aea419e0a8a32`; and
`replay` restored the latter digest and was a semantic no-op. Every stage exited
`0`, both runtime-claimant checks remained zero, and every process, profile,
log, spill, and guard-root cleanup counter was zero.

This proof uses a guarded deterministic no-nearby-player projection seam. It
does not certify live player proximity or materialization, physical or
dematerializing restart cuts, prepared-settlement crash cuts, native
persistence-source selection, packaged or live dedicated server/client
behavior, migration, marker runtime, multiplayer/JIP/reconnect, performance, or
soak. Its fingerprint also does not certify live source/target owner revisions
or the absence of operation-owned ownership-transition claimants.

The stamped checkpoint passes Foundation at 814 references. Guarded Workbench
validation passes 5,832 files and 11,828 classes at CRC `92fcd4a4`, reports
`ScriptValidation true`, exits `0`, records zero hard errors, and leaves every
cleanup counter at zero.
...
| Exact enemy counterattack | Schema 69 gives new contract-`1` counterattacks one reciprocal order/operation/manifest/batch/group aggregate, one charged attack or support pool, direct virtual travel, deterministic combat, physical/virtual casualty continuity, canonical ownership transition, return, and proportional survivor refund. The appended `PREPARED` settlement intent enforces prepare, stage, refund, record, and finalize ordering across restore and same-session retry; derived-ID claimant scans reject ambiguous residue. The native proof drives one focal order through production-owned `VIRTUAL` -> `MATERIALIZING` -> `PHYSICAL` -> `VIRTUAL`, then through one real engine death, adapter-owned slot retirement, N-1 fold, N-1 re-entry, and final N-1 fold. The fresh-process proof separately validates one canonical fallback outbound-`VIRTUAL` reciprocal graph across prepare, deterministic recover continuation, and semantic-no-op replay, with exact readback and zero adapter/PhysicalWar runtime claimants. Historical rows remain contract `0`; malformed Schema-69 graphs quarantine at `-69` without fabrication, deletion, settlement, refund, or outcome. | The current three-process proof preserves digest `046514a9170db409`, advances it exactly to `1d2aea419e0a8a32` during recovery, and leaves that digest unchanged during replay. All stages exit `0`; all process/profile/log/spill cleanup counters are zero. Foundation passes at 814 and guarded Workbench passes 5,832 files/11,828 classes at CRC `92fcd4a4`. This closes only canonical-fallback strategic restart under the deterministic no-nearby-player seam. Live proximity/materialization, physical/dematerializing and prepared-settlement cuts, native persistence source, package/live server-client, migration, markers, multiplayer/JIP/reconnect, performance, and soak remain open. The fingerprint excludes live endpoint owner revisions and operation-owned ownership-transition claimant absence. |
...
| Certification | Schema 70/settings 24 remains the current persisted contract. Current implementation `339b72ec3ed63132e46f3df84540d74d3e938d16`, stamp `7736b42`, UTC `2026-07-15T22:32:02Z`, label `schema70-settings24-counterattack-virtual-restart-proof`, adds no schema/settings migration. | Foundation passes at 814 references. Guarded Workbench loads 5,832 files/11,828 classes at CRC `92fcd4a4`, reports `ScriptValidation true`, exits `0`, records zero hard errors, and cleans exactly. The exact fresh-process prepare/recover/replay proof passes with deterministic continuation, semantic-no-op replay, zero runtime claimants, and exact cleanup. This is targeted proof, not campaign certification: native persistence source, world scope, physical and settlement restart cuts, package/live server-client, migration, markers/rendered UI, multiplayer/JIP/reconnect, performance, soak, and unrelated suite failures remain open. |
