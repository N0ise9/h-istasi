...
package files and 50 evidence files. Package-bound runtime proof remains
pending.
...
observable mixed state. No runtime evidence transfers. `STATUS-008` remains
open, Gate 1 is incomplete, and release stays `NO-GO`.
...
1. Run and independently accept a fresh surface audit plus diagnostic-write/
   standard-read runtime-retention evidence under the canonical publishers,
   then consume the pair. Neither half alone advances Gate 1; `STATUS-008`
   stays open until both are accepted together.
