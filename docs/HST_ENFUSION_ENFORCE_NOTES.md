...
- Keep live inventory payloads single-owner.
  ...
  - A `BaseLoadoutClothComponent` alone is not enough to exempt an item from structural-container blocking. Reforger's own placement checks mention cloth components while avoiding pouch detection; only bypass structural probes for actual loadout clothing categories or known wearable backpacks/field packs.
