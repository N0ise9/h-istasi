...
counterattacks use contract `1` and one reciprocal order/operation/manifest/
...
Source work has entered Blueprint Phase 9 of 13 through Schema 69, whose scoped
...
| --- | --- | --- | --- | --- | --- |
| CRI-0 Truth and baseline | Campaign Schema 69/runtime-settings Schema 24 is the current contract; its scoped engine-proof checkpoint is sealed at implementation `5bdcda938840ab769b41ff3e1856d908572a8c45`, label `schema69-settings24-exact-enemy-counterattack-engine-proof`, with stamp commit `73a64ef` | Schema 69 adds one exact counterattack aggregate without changing historical contract-zero rows | Foundation 771, all-target Workbench log `logs_2026-07-13_15-41-50` at CRC `3a8bd64f`, and focused engine log `logs_2026-07-13_15-42-52` with one passing JUnit testcase, an empty failed list, and `AllExact=1` pass. The focused environment also records a recoverable base-game VM exception before successful HST completion | No | Execute Full Campaign Debug Phase 17, then package-prove serialization/restart, fresh and affected-save authority, profile migration, marker behavior, dedicated/live-server, networking/JIP/reconnect, and soak behavior. |
...
| CRI-2 Force manifests | Complete for the sealed foundation and scoped Schema-69 engine proof | Durable SpawnQueue and exact infantry adapters retain the sealed consumers; the counterattack slice adds one frozen infantry manifest without widening vehicle, asset, or multi-root admission | Foundation, all-target Workbench, and focused deterministic admission/roster/projection assertions pass; native/package/restart behavior remains unproved | No | Package-prove that the roster remains frozen through live casualties, virtual/physical transfer, re-entry, and restart without refill, while historical contract-zero counterattacks remain isolated. |
| CRI-3 Force runtime | Complete for sealed source/engine proof; runtime certification open | Existing casualty/reprojection paths remain. Schema 69 adds counterattack direct travel, physical realization/fold, deterministic virtual combat, return, and survivor settlement over one durable roster | Schema-69 Full Campaign Debug and native/package/restart gates are pending | No | Prove live counterattack casualties, fold/root retirement, held-slot continuity, re-entry, ownership retry, return, and restart alongside every earlier family. |
| CRI-4 Operation records | Scoped Schema-69 engine proof sealed with appended counterattack and `PREPARED` values | Schema 69 adds a separately typed exact counterattack and durable terminal intent without changing earlier enum values or contracts. Settlement follows prepare -> stage -> refund -> record -> finalize and resumes on restore or same-session retry | Foundation gates explicit and derived-ID claimant scans. The focused engine proves PREPARED recovery, same-session ABORTED recovery, duplicate and foreign derived-ID collision hold, and fail-closed SETTLED state without a resource receipt | No | Execute the reciprocal graph, terminal cleanup, historical isolation, and current `-69` quarantine through Full Campaign Debug and packaged serialization/restart. |
...
| Durable operation state | Every force/support/order has an immutable assignment, duty state, engagement mode, materialization state, route progress where applicable, recall/return policy, and terminal result. | Sealed nine-type/eleven-consumer foundation plus scoped Schema-69 engine proof | The appended counterattack type adds direct route, deterministic combat, canonical ownership, return, and one-pool settlement over one frozen roster. The appended `PREPARED` settlement state resumes prepare/stage/refund/record/finalize across restore and same-session retry. Focused proof passes; Full Campaign Debug, packaged runtime, generalized encounters, vehicle/multi-root policy, JIP, historical missions/garrisons, and other families remain open. | Highest |
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Versioned save state | Durable campaign facts survive restarts and schema migration. | Implemented Foundation / Scoped Schema-69 Engine-Proof Restore Contract Sealed | Schema 69 adds no fields, preserves historical counterattacks at contract `0`, validates current exact aggregates before generic projection normalization, and resumes valid PREPARED settlement prefixes. Focused restore/replay proof passes; real serialization/restart remains open. | Highest |
...
| Enemy commander orders | Counterattacks, rebuilds, roadblocks, support calls, and HQ pressure queue durable orders. | Scoped Schema-69 exact-counterattack engine proof / Sealed Schema-68 planning | Periodic planning retains its sealed commitment-aware behavior. Newly admitted counterattacks use contract `1`, one frozen infantry manifest, one direct route, and one exact operation graph; historical restored counterattacks remain contract `0`. Foundation, all-target Workbench, and focused proof pass. Execute Full Campaign Debug, then package-prove both the new lifecycle and the earlier planning/restart gates. | Highest |
| Abstract resolution | Off-screen orders and support resolve without needing physical entities. | Scoped exact-counterattack engine proof plus source-complete exact QRF/patrol routes | The counterattack slice carries the same surviving slots through direct virtual travel and deterministic virtual combat, then continues return travel after the canonical ownership result. It must not estimate a fresh roster on re-entry. Full Campaign Debug, save/restart, package, and soak proof remain open. | High |
| Physical response | Near-player enemy responses spawn, move, fight, and fold back. | Schema-69 source/engine proof sealed / Needs runtime proof | Exact counterattack members materialize from the frozen roster, fold confirmed casualties back to virtual authority, and may resume strategic combat outside the bubble. Ownership publication waits for canonical transition authority, and settlement waits for return. Native spawn/contact/fold/re-entry/casualty, marker, restart, and multiplayer proof remain open. | Highest |
...
| Full Campaign Debug | One button runs a true runtime certification suite and writes structured artifacts without changing the campaign under test. | Broad Alpha / Schema-67 And Schema-68 Fixtures Wired / Scoped Schema-69 Focused Autotest Passed / Needs Full-Suite And Cross-Domain Runtime Proof | Schema 67 resource assertions and 17 Schema-68 planning fixtures remain wired. Schema-69 focused case `HST_TEST_EnemyCounterattackAuthority` passes in log `logs_2026-07-13_15-42-52`: one JUnit testcase, empty failed list, and `AllExact=1` across planning, admission, travel, combat, projection, ownership, settlement, restore, resource authority, ambiguity, quarantine, and retention. Valid PREPARED and same-session ABORTED recovery, foreign derived-ID collision hold, and SETTLED-without-resource-receipt fail-closed proof pass. The environment also writes a recoverable base-game VM exception before the HST case completes successfully. This did not execute Full Campaign Debug in `HST_Dev`, coordinator isolation/artifacts, live campaign authority, persistence/restart, native world behavior, package, networking, or soak. | Highest |
...
1. Execute the Schema-69 exact-counterattack Full Campaign Debug Phase-17 proof
   now that Foundation, all-target Workbench, and the focused engine case are
   green. Cover one-pool admission, frozen roster/route authority,
   virtual and physical casualty continuity, deterministic combat, canonical
   ownership retry, PREPARED settlement resume, return, proportional survivor
   refund, contract-zero isolation, derived-ID ambiguity, `-69` quarantine
   idempotency, and leak-free cleanup. Then package-prove serialization/restart,
   markers, multiplayer, and soak behavior. Execute the still-open Schema-68
   commitment-aware Campaign Debug and live-server gates in the same controlled
