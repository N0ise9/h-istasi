...
The current development save contract is unsealed Campaign Schema 65 with
runtime settings still at Schema 24. This Blueprint Phase 8 civilian-consequence
...
perform real profile I/O. Preliminary unstamped normal Workbench compile/create
and all-five validation are clean at 5,802 Game files/11,728 classes with CRC
`be076102`, `Script validation successful`, zero HST script errors, and zero
surviving Workbench processes. The current tree has no sealed Schema-65
identity. Final Foundation, stamped Workbench reruns, Campaign Debug execution,
...
Schema 64/runtime-settings Schema 24 under implementation
...
surviving Workbench processes. No new stamp or package exists yet.
...
The current development save contract is unsealed Schema 65. It retains Schema
64's one canonical town record for FIA/occupier/invader support, population,
...
fails closed through the existing `-64` boundary.
...
| --- | --- | --- | --- | --- | --- |
| CRI-0 Truth and baseline | The current development tree is unsealed Campaign Schema 65/runtime-settings Schema 24. The last sealed/publishable checkpoint is Schema 64/settings 24; the Schema-23 canonical-town checkpoint precedes it | Schema 65 adds bounded exact civilian casualty/theft queues, exact-pilot durable-promotion theft, a current-fact combat episode envelope, persistent political/aggression consequences, indexed save validation/migration, and pedestrian panic/recovery without making ambient topology durable | The new deterministic state-proof suites are complete and wired but have no native or real profile-I/O result. Preliminary unstamped normal compile/create and all-five validation are clean at 5,802 Game files/11,728 classes with CRC `be076102`, `Script validation successful`, zero HST errors, and zero surviving processes. Last sealed evidence remains Foundation 711 and normal/all-five Workbench checks at 5,799 files/11,718 classes with CRC `bb083672` | No | No Schema-65 stamp/package exists. Final Foundation and stamped Workbench reruns remain open. Until a new stamp is created, only `schema64-settings24-ambient-runtime-authority` is publishable; package/native/save-restart/rendered-UI/ten-town soak/setup/network/reconnect/JIP gates remain independently open. |
...
| CRI-8 Civilian runtime and political consequences | Ambient and first civilian-consequence authority are designed | Sealed Settings-24 ambience provides fair budgets/lifecycle/save boundaries. Unsealed Schema 65 adds 256-casualty/64-theft queues, a combined four-attempt frame cap, bounded-backoff indefinite retry and capture deferral, exact-pilot theft after durable promotion, combat danger only from current-operation/recent-fire facts, at most one canonical-town political event per persisted episode edge, adopted-floor/last-applied invariants, and physical pedestrian `Wandering -> Panicked -> Recovering` with separate bounded panic-route recovery. Minor localities are panic-only with session-only receipt fingerprints | Deterministic state proofs are complete and wired, and preliminary unstamped normal/all-five Workbench checks are clean at 5,802 Game files/11,728 classes with CRC `be076102`; final Foundation/stamped reruns plus native callback, movement, and real save/restart execution are unverified | No | Native-prove queue capacity/ordering/attribution and capture deferral, pilot-only promotion/theft with passenger protection, `HOT`-only inertness, pending-receipt drain before a new edge, logical consequence continuity without actors, adopted-floor/restart linkage, minor-locality receipt limitation, panic/recovery without hot-path AI activation, ten-town lifecycle/capture/reset/restoration, and two-nearby-same-prefab no-collapse. Aid and ownership/security-pressure also need runtime proof; deeper local security remains open. |
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Versioned save state | Durable campaign facts survive restarts and schema migration. | Implemented Foundation / Unsealed Schema-65 Source Boundary | Schema 65 adds the persisted locality consequence episode/adopted-floor/last-applied envelope and exact town-event aggression before/after evidence. Schema-64 migration derives only an adopted applied floor (`0..1`) from existing current-operation/recent-fire facts, clears unavailable legacy aggression evidence, and creates/replays no consequence or strategic event. Indexed restore validates structural pool/receipt chains and exact live-town backlinks; a preset-aware post-restore pass separately quarantines aggression aimed outside configured enemy roles. Current invalid envelopes quarantine at `-65`; inconsistent town-event authority retains Schema 64's `-64` fail-close. Real serialization/restart remains open. | Highest |
| Runtime settings migration | Generated profile settings migrate forward without keeping obsolete setup knobs. | Implemented Foundation / Sealed Settings Schema 24 Source/Workbench | The `23 -> 24` migration adds normalized global civilian actor and traffic budgets, per-player additions, war-level budget penalty, runtime health/startup/stuck/recovery/backoff controls, and preserves explicit older values. Campaign persistence independently advances to unsealed Schema 65; generated settings remain Schema 24. Generated profile rewrite and packaged migration proof are pending. | Keep |
...
| Full Campaign Debug | One button runs a true runtime certification suite and writes structured artifacts without changing the campaign under test. | Broad Alpha / Schema-64 Plus Unsealed Schema-65 Fixtures Wired / Needs Runtime Proof | Schema 65 adds casualty attribution/replay, theft, combat-episode and `HOT`-only gates, minor-locality panic-only, malformed/aggression/persistence, and pedestrian panic/recovery assertions beside the Schema-64 town suite. The state-only proof cases are complete and wired but have no native Campaign Debug artifact. They do not substitute for native entities, real restart, rendered UI, networking, reconnect, JIP, or mandatory process restart. | Highest |
...
| Performance soak | Physical/abstract transitions do not leave stuck groups, duplicate vehicles, missing markers, or periodic frame stalls. | Needs Soak / Sealed Schema-64 Repair Plus Unsealed Schema-65 Bounded Consequences | The latest user run showed a visible once-per-second stutter. Schema 64 keeps influence aggregates incremental and throttles active-group logs; Schema 65 adds bounded queues/state iteration and consumes indexed combat facts rather than a hot-path world scan. None is runtime profiling evidence. Do not publish the unsealed tree under the old stamp; profile the exact Schema-65 identity only after it is sealed, and measure freezes per minute before calling the stutter fixed. | Highest |

## Highest-Impact Next Tasks

1. Do not publish the unsealed Schema-65 tree under the old stamp. Until a new
   stamp exists, the last publishable identity remains exact sealed Campaign
   Schema-64/runtime-settings Schema-24
   `schema64-settings24-ambient-runtime-authority`. Require any evidence claimed
   for that older checkpoint to carry its exact `HST_BuildInfo` identity.
...
3. Runtime-prove civilian projection across at least ten simultaneously eligible
   ...
   only and cannot create political history.
