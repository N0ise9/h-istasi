...
| `Scripts/Game/HST/Components/HST_SetupMapComponent.c` | Native map layout creation, `ScreenToWorld`, centralized modal/prompt z-order, passive prompt flags | Allowed with guardrails | `ScreenToWorld` is allowed only after viewport readiness checks. Keep modal buttons widget-driven. Continue avoiding setup zone publish during setup. |
