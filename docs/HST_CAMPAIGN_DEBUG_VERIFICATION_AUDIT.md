...
Gate 1 is failed; earlier evidence does not transfer and release remains
`NO-GO`.
...
hash-bound artifacts, stable source/toolchain bindings, exact suite-isolated
cleanup, and zero final engine processes. Their tracked summary SHA-256 is
`9d7241d198b6ebb2613a1e7ec1b656b460ceb531d02b0a4ebe35ec239a3da455`.
The source-native force-authority canary also passes as scoped noncertifying
proof: 9 PASS/2 WARN/0 FAIL/0 BLOCKED/0 SKIPPED, 35/35 focused assertions,
87/87 counted conditions, 18 ordered zero-delta state rows, two approved and
zero unapproved hard diagnostics, zero final orphans, and zero residual engine
processes. Its tracked summary SHA-256 is
...
passed at 5,849 files/12,022 classes with common CRC `aeddce9b`, zero hard
...
The rejected focused leaf cannot enter the aggregate. The completed tool gate is
15/15 source, 56 runner checks, 74 publisher checks with 73 negatives and two
guarded receipts, 73/73 retention checks, 3 valid/optional plus 53 adversarial
consumer cases, and a 741-check focused-aggregate self-test.

Clean harness `e2c38d2770d8ebaaa675326d1b8a91068db989e5` produced surface leaf
...
Its structured candidate-mount attestation is exact at 2/2/1/0. Retention leaf
`20260722T103514Z-436f331b8659` retained 251 files; index and ready SHA-256
values are `94906067a7637e69a186926dc731320cf14011a04c9fe7a26117960f46e7f29a`
and `ff503f5f1f16ad7fbac45c292ad43a2b07984ca35ce8a188cbee2b55ddaea503`.
...
CRI-101 records the fresh pair under clean harness
`7e2fe71d9619fd60cfe393d2776b4ed49f75186b`. Surface leaf
`20260722T114802Z-1fb5e08619ef45b2aae20f2c5938b56c` retains 41 files; its
tracked index and ready-seal SHA-256 values are
...
Both retail and diagnostic candidate-mount attestations are exact at 2/2/1/0.
Retail retains the exact approved 6-raw/2-event diagnostic cluster, diagnostic
retains exact 0 raw/0 events, and neither mode has an unapproved diagnostic.

Retention leaf `20260722T115003Z-4e159a24384c` retains 251 files; its tracked
...
It covers five diagnostic plus five standard contexts and retains the exact
noncertifying boundary `standardSaveRestorationCertified: false` and
`certificationClaim: none`. Both halves independently reverified read-only and were
jointly consumed against unchanged package
`af22d6322a215dbef466e49041fc07395cbb5ed7a5951fd3e0cee5f4a101f530`
and candidate binding
`06b5fa23ccd10bfd1f8621f6b03c25d9c36719cc9c7d08ebcc73dabbba8c9f5f`.

`STATUS-008` is closed again. The `52c7e2b` and
`e2c38d2770d8ebaaa675326d1b8a91068db989e5` pairs remain immutable passed
history under their recorded tool bytes. Focused leaves
...
Before the current activation, the sealed
`ee0e8add2a298e83fd304b7660c4fc480dc6383f` tree was the last documented
all-target Workbench success in this history. A guarded PC control
...
`partisan-rc-e11e7ea88a44-20260719T040154Z` as `history[1]` with
`rejected-after-corrected-canary`. The e11 entry retains its own focused and
rejected corrected-canary evidence and has no `fullCampaignDebug` property.
`partisan-rc-ee0e8add2a29-20260719T063815Z` is `history[2]` with
`rejected-after-full-profile` and retains its own focused, accepted corrected-
canary, and rejected full-profile results unchanged. The checked activation
replaced the entire current-candidate surface with the 5b1f candidate without
...
fail-closed evidence for that correction. CRI-101 then accepts surface run
`20260722T114802Z-1fb5e08619ef45b2aae20f2c5938b56c` and retention run
`20260722T115003Z-4e159a24384c` under clean harness
`7e2fe71d9619fd60cfe393d2776b4ed49f75186b`. Their 41- and 251-file bundles
...
as superseded artifact evidence. The earlier candidate,
...
only to that package. Historical candidate,
...
exact package. Historical e11 and ee0 evidence is detailed below. No result may
cross any package boundary into the active 5b1f chain.

## Historical `ee0e8add2a29` Packaged Focused Boundary
...
the exact then-active ee0 candidate and packed mount. All five passed with aggregate
...
independently accepted at the corrected-canary boundary below; the subsequent
full profile is independently rejected at the boundary after it.

## Historical `ee0e8add2a29` Accepted Corrected-Canary Boundary

Clean harness HEAD `4f8d7e2d7a39896737fd6754060523bf852c5fa8` ran
`seed1985_t0_p1_u1784445266` against the exact then-active candidate
...
performance, soak, canary-release, or stable-release gates. The full result
below is rejected red; release remains `NO-GO`.

## Historical `ee0e8add2a29` Rejected Full Campaign Debug Boundary

Clean harness HEAD `a5ccf36aee17a4f88d7f1c2f232ce9fc14652018` ran
`seed1985_t0_p1_u1784446076`, leaf
`20260719T072739Z-97fc069d58cd427c848c83f99f39e5f9`, against the exact then-active
candidate `partisan-rc-ee0e8add2a29-20260719T063815Z` and exact package SHA-256
`981258439b9d08866c4883471cacfe33aa373a36a667a39e8c939f285db74daf` from
`2026-07-19T07:27:39.1454367Z` through `2026-07-19T07:40:09.5714410Z`.
Candidate binding, packed mount, package bytes, and artifacts were exact and
stable. All ten envelope files rehashed to envelope SHA-256
`fce4928444f15531f254ad4d7e119cf8bfe1d06e6fcb564518d2e052544d4278`.
State restoration was 18/0, Phase 17 projection checks passed 11/11, Phase 24
passed 2/2, staged cleanup passed 6/6, final orphan cleanup passed with zero
active groups, and every cleanup/spill counter was zero.

The mechanically valid capture is not an accepted result. Across 685 cases the
full report records 598 PASS, 47 WARN, 26 FAIL, 13 BLOCKED, and 1 SKIPPED. It
proves 5,630/5,695 certification-required assertions, leaving 50 failed and 15
blocked. Certification therefore fails independently of the diagnostic audit.
The audit also fails independently: 26 hard diagnostics classify as two
approved stock, zero approved intentional, and 24 unapproved: 22 Partisan and
two runtime. The unapproved
set has three exact clusters. An obsolete classifier expectation for a
fourteenth intentional convoy diagnostic demotes the 13 valid intentional-
negative convoy diagnostics; nine debug respawn-race errors form the next
cluster; and two HQ arsenal teardown errors form the final cluster. The
portable summary is
...
five-second cadence, and the 45-second grace. These fixes are present in the
active candidate but do not change any rejected ee0 row or transfer its proof.
...
`history[1]` as `rejected-after-corrected-canary` and remains uncertified. Under
clean harness HEAD
`b1940f241e28f163202807385f7140f048921447`, all five
...
corrected canary is rejected at 33/35 focused assertions and 85/87 counted
conditions, so scoped native-engine/world is `failed` and Full Campaign Debug
is stopped for this package; no prior-package result is carried forward.
...
cleanup. The seventeen-check consumer test admits all three dispositions for
...
Clean harness HEAD `937c86c5d2259a9da270ea76371001ac1d4c6eed` ran the corrected
`force_authority` profile against the exact then-active candidate. Run
`seed1985_t0_p1_u1784437399` started at
`2026-07-19T05:03:02.0611638Z` and completed at
`2026-07-19T05:03:41.5393020Z`. Across 11 cases the report was 8 PASS, 1 WARN,
1 FAIL, 1 BLOCKED, and 0 SKIPPED. The focused case failed with 33/35 assertions
and 85/87 certification-counting conditions proven. The two failed assertion
IDs were `ownership_transition.aggregate` and `ownership_transition.causes`.
Exact cause evidence was military/political/mission/admin/debug/migration
1/1/0/1/1/1. Serialized intent evidence was queued 0, restore 1, political
exact-once 0, and repeat/restart 1/1.

The wrapper armed, started, and completed. Candidate boundary, exact packed
mount, and retained artifact stability were valid. The hard-diagnostic census
was two approved stock + zero approved intentional + zero unapproved; all 33
classifier checks passed. All ten envelope files rehashed, the state diff was
18/0, final orphan cleanup passed with zero active groups, and every cleanup and
spill counter was zero. Envelope SHA-256 is
`8deca62633394025bfa976f6d883f9b500d56519fd13e875f241679f4799cd21`.
The tracked portable summary is
`docs/evidence/campaign-debug/partisan-rc-e11e7ea88a44-20260719T040154Z-corrected-canary-20260719T050302Z.json`,
SHA-256 `af0aca25a84d8f757dbba8010950a658ce09937aa4048c35b2e372f1183eec69`.

Capture integrity does not make the failed proof acceptable. The canary is
rejected, native-engine/world is `failed` for this package, and the full profile
was correctly not run. Its historical disposition makes it verification-only
and runtime-ineligible. The fixture correction is sealed in historical ee0,
whose focused and corrected-canary rungs later passed before its full profile
was rejected.

The reproduced root is confined to the proof fixture. Production correctly
uses `FindActiveMission` as the prerequisite for classifying a supplied source
as mission provenance. The stale all-cause and serialized-retry paths supplied
mission-shaped IDs without seeding the exact active missions, so mission cause
was 0 and political exact-once was reported 0 through a combined diagnostic.
Current source retains the production guard. Both proof paths now seed the exact
active mission and assert cause, source type, and source ID. The all-cause path
also proves that an unknown source stays `military_capture` with
`zone_capture` provenance, while serialized evidence reports political exact-
once separately from mission provenance. Narrow Foundation guards require the
active-mission factory, both exact provenance checks, the negative control, and
the split diagnostic. This correction is sealed in ee0 and transfers no evidence
to the rejected e11 package.

A first all-five focused execution on the prior candidate produced one passing
JUnit result per named
...
`passed-noncertifying`; it is not evidence for the retained rejected replacement.
...
## Historical Post-Boundary Correction Checkpoint
...
the complete batch was sealed into the then-active replacement. The rejected result
...
Workbench, and sealed-package gates passed on the retained replacement; its
later corrected-canary boundary is rejected below.
...
Foundation and guarded PC compile gate recorded below and was sealed in the
then-active replacement; its later corrected-canary boundary is rejected below.
...
recorded below and was sealed in the then-active replacement; its later runtime
...
`passed-noncertifying`. The corrected canary boundary above is rejected at
33/35 and 85/87, so native-engine/world is `failed` and the full profile is
...
historical ee0. That candidate's focused and corrected-canary rungs pass as
noncertifying evidence, while its independently executed full profile is
rejected red at 598/47/26/13/1 and 5,630/5,695 with 24 unapproved diagnostics.
...
for that package. Retained ee0 passes both focused and corrected-canary gates as
noncertifying evidence; its full profile is now independently rejected red at
598/47/26/13/1 and 5,630/5,695 with 24 unapproved diagnostics. The candidate
remains immutable and release remains `NO-GO`.
...
full-profile boundary, which are also historical. The historical e11 rejected
replacement did not inherit either package's focused, canary, or full results.
Its own five packaged
...
sealed in historical ee0, whose focused and corrected-canary rungs pass without
evidence transfer as noncertifying evidence. Its mechanically exact full
profile is independently rejected at 598/47/26/13/1 and 5,630/5,695 with 24
unapproved diagnostics; source fixes require a new candidate.

## Historical Focused Force-Authority Engine Checkpoint

The retained non-package `force_authority` engine profile passed its registered
case and all 35 targeted assertions: combat presence 9/9, ownership transitions
14/14, and town influence 12/12. The report counted 87/87 exact conditions, all
18 tracked state-diff rows remained zero, and the guarded run reported zero
script or Partisan errors, zero crashes, stable artifacts, and exact-zero owned
cleanup.
...
Blocking `SHUTDOWN` is now an ordered, one-way transaction across every live
save owner. Before the first latch, persistence runs read-only readiness for
nearby durable vehicles, rescue authority, field/civilian vehicles, and active
groups, runs complete capture preparation, then repeats the complete prepared-
graph and player preflight. Only after that clean boundary may it latch nearby
vehicle scope, repeat preparation, and latch or maintain active-group, vehicle,
and rescue scopes in order. Once a scope is latched, retry maintains its
original pins and may not reopen it or capture a newer graph.

The coordinator rejects commander/member/admin and mission/casualty/lifecycle
ingress while draining or quiescing. Rescue pins serialized DTO authority plus
the native captive/follower/carrier/compartment/seat/occupancy/player/proxy/
damage topology and full carrier transform. Foreign or player occupancy is
rejected before latching, and canonical player-release evidence is required.
Follower callbacks, waypoints, AI, movement, and recursive physics are
quiesced, as are group/vehicle controllers, engine, brake, autohover,
velocities, and recursive physics. Retry reapplies pinned transforms while
rejecting entity, group, member, seat, occupancy, token, player, or damage
identity drift.

The mixed-native five-stage regression passes autosave, manual, controlled
shutdown, native no-save verification, and profile-fallback no-save
verification. Before any latch, the shutdown fixture seats an unregistered
foreign occupant and proves rejection plus exact disembark/deletion. It then
proves a connected player occupant is rejected with canonical player-release
evidence. Both rejections leave all one-way latches clear; after player release,
the normal controlled-end retry applies the ordered latches and subsequent
attempts maintain the original pins rather than recapturing authority.

The exact rescue graph survives with one `FOLLOWING` captive, one seatless
`BOARDING` captive, and one `BOARDED` captive in the same authored stable cargo
seat. The carrier's durable `vehicle_<old RplId>` identity intentionally remains
unchanged across restart while a genuinely read-only binding resolver proves
that key maps one-to-one to the fresh replicated root. The logical fingerprint
uses the frozen shutdown pose; current durable and live carrier/captive poses
are independently required within 3 meters and 3 degrees.

Restored guard authority releases the exact held batch, runs adapter and routed-
group reconciliation, and samples `PHYSICAL` authority before ordinary player-
distance folding can return it to strategic hold. Shutdown, native restore, and
profile-fallback restore each retain exactly 3 captives, 1 carrier, 1 active
group, 2 living guards, and 3 adapter handles. Generations advance 1 -> 2 -> 3;
both no-save verifiers leave persistent proof authority read-only; and all
guarded process/profile/
proof/spill cleanup counters are exactly zero.

This closes the scoped mixed-native controlled-shutdown and fresh-process
restart fixture. Packaged live multiplayer, JIP, reconnect, broader failure
injection, performance, and long-soak certification remain open.

## Current Exact Garrison-Rebuild Fresh-Process Checkpoint

The exact enemy garrison-rebuild proof carries both `delivery_pending` and
`physical_live_fold` cuts through three separate dedicated-server processes.
Every process is bound to the same disposable profile owner,
build/schema/world/run/cut identity, and immutable prepared carrier. Each stage
receives a fresh lowercase-hex nonce in a one-use guard. Coordinator startup
validates the profile owner and shape, consumes and deletes that guard, then
loads the carrier before migration or restore can mutate proof authority.

For `recover` and `replay`, startup reads and validates the selected campaign
JSON snapshot, validates its pending or delivered semantic fingerprint, adopts
that exact state as `m_State`, and tracks it before any schema validator or
operation `ReconcileAfterRestore()` call. The `recover` stage advances the
remaining virtual route through production `TickOrder()` and
`CompleteDelivery()`. It proves one continuous casualty ledger, one exact
delivery receipt, one held-garrison manifest link, no aggregate double count,
one original support debit plus its exact zero-refund delivery mutation, and no
physical claimant in that virtual cut. The carrier remains unchanged.

The `physical_live_fold` cut begins with the same nine accepted members, eight
living members, one prior casualty, and partial route progress. It proves
`VIRTUAL -> MATERIALIZING -> PHYSICAL/LIVE` with exactly one native root, nine
adapter handles, eight living native members, ten live position samples, 2.759
meters of movement, and 0.539 meters of target closure. It then drives the
production `PHYSICAL -> DEMATERIALIZING -> VIRTUAL` fold, retains the exact
8-living/1-casualty authority with no remaining native runtime, persists the
fold, and proves exact restart/replay behavior.

A virtual delivered row must use the same durable statuses at all three
boundaries: `CompleteDelivery()`, save normalization, and
`ReconcileAfterRestore()`. The order status is
`resolved_exact_rebuild_virtual_on_station`; the group status is
`enemy_garrison_rebuild_virtual_on_station`. Any spelling or lifecycle drift
between those boundaries turns a valid restored row into a false mutation and
breaks semantic replay. `ReconcileAfterRestore()` therefore accumulates
`changed` only when it actually normalizes a field, resumes a settlement, or
quarantines invalid authority. Merely visiting a valid exact row returns no
change.

The journal proof also corrects a filename assumption. `HST_CampaignSaveData.json`
and `HST_CampaignSaveData.recovery.json` are alternating slots; neither filename
means "newest." A fresh `prepare` writes generation 1 to the first filename.
`recover` retains those bytes and writes generation 2 to the recovery filename,
whose parent generation and fingerprint point to generation 1. Selection must
follow verified envelope generation/parent/fingerprint authority rather than a
preferred filename. `replay` captures file signatures for both slots and the
carrier before launch, requires both journal generations, and proves all three
files are byte-identical afterward. It performs no checkpoint or carrier write.

Both final stamped cuts report `a8e261d00e13`. The `delivery_pending` summary
records prepared digest `6fdb9d3e45a08b92`, delivered/replay digest
`16b17e6617027292`, generation `1 -> 2`, exact source/continuation/receipt/held-
garrison/resource flags, and semantic/journal/carrier read-only replay. The
`physical_live_fold` summary records prepared digest `f61c9b7d9671d1af`,
delivered/replay digest `f6e11ac1ebcfe3d8`, exact physical bindings/movement/
fold, and the same exact generation/read-only replay contract. Every cleanup
counter is zero in both runs.

Failure reporting now preserves root cause ahead of cleanup. A failed stage
front-loads compact source/restore/reconcile/continuation/receipt/resource/
physical/progress flags and bounded sanitized stage evidence. The wrapper then
runs its exact cleanup census; if cleanup also fails, it prints the earlier
...
certify natural route/combat variation, other force families, arbitrary native
session-save selection, packaged live gameplay, multiplayer/JIP/reconnect,
performance, or soak.

## Current Campaign Profile Journal Authority Checkpoint

Campaign JSON is now a two-slot recovery journal rather than one directly
overwritten mirror. The canonical and recovery slots contain a versioned
envelope whose payload is the exact serialized campaign DTO string. Each
envelope records generation, parent generation and fingerprint, payload schema,
and a `uuidv8-sha256-v1:<serialized-length>:<UUID>` payload fingerprint. The
fingerprint detects corruption and establishes exact identity; it is not an
authentication mechanism.

Production alternates into the inactive slot, reads it back, and re-runs the
same selection logic before accepting it. The previously selected valid slot is
not overwritten until its replacement verifies. This gives two-generation
crash tolerance but is not an atomic filesystem transaction: there is no lock,
atomic rename, multi-writer guarantee, authenticated storage, or off-device
backup. The canonical/recovery names identify alternating files, not newest and
backup roles; envelope generation and parent fingerprint establish ordering. A
raw Schema-70-or-earlier canonical file is generation 0; the first
Schema-71 checkpoint writes generation 1 into recovery while preserving the raw
canonical bytes.

Native and profile authority share one durable order: checkpoint sequence,
restore sequence, then last-save second. Equal-order candidates require the same
comparable normalized payload fingerprint. A newer valid journal can therefore
recover past stale native state, while newer valid native state wins over a
stale journal. No native authority uses the journal. An unusable native row may
enter explicit degraded journal recovery, after which the session remains
profile-only. Unsupported-future native authority is fatal before journal
selection. With no valid native source, present invalid journal artifacts are
fatal; only absence of both sources creates a new campaign. A valid native row
may continue through read-only startup when journal artifacts are invalid or
future. A later verified native checkpoint may repair an ordinary invalid or
truncated inactive slot. Unsupported-future or ambiguous/conflicting journal
history fences writes and preserves the artifacts. Split brain, broken parent
links between otherwise valid generations, future envelope/payload versions,
ambiguous duplicate metadata, and
equal-order fingerprint conflicts all fail closed as journal authority.

The native scripted-state envelope now writes version 2 with an exact serialized
...
- `HST_TEST_CampaignProfileJournalAuthority` passes 1/1 with zero failures,
  errors, or skips and an empty failed list. Its 41/41 exact booleans cover
  alternating generation/readback, raw-generation-0 migration, damaged-latest
  fallback, replacement after damage, split brain, broken chain, future magic/
  schema/payload fencing, duplicate-metadata ambiguity, writer non-mutation,
  rich nested payload identity, legacy/current native fingerprint
  normalization, and the production native/journal routing seam. Native-v1,
  native-v2, invalid-fingerprint, and future-envelope classifier cases are each
  exactly 1/1.
- The ordinary five-process chain passes 5/5 across periodic `AUTO`, typed
  `MANUAL`, real blocking `SHUTDOWN`, native no-save restore, and profile-journal
  no-save restore. It advances generations 1 -> 2 -> 3, selects canonical
  generation 3 with two valid slots and an exact chain, keeps both restore
  verifiers read-only, preserves exact field-vehicle authority, and retains the
  mixed-native 3-captive/1-carrier/1-group/2-living/3-handle graph. Foreign and
  player occupancy rejection/release, one-way retry, the authored stable seat,
  old-ID-to-fresh-root binding, frozen fingerprint, bounded pose comparison, and
  guard rematerialization ordering are all exact; every cleanup counter is zero.
...
- The dedicated three-process `prepare_old_checkpoint -> reset_commit ->
  ...
  authority is `cp6/r2`. The final process selects the newer profile fallback,
  enters degraded newer-journal recovery, performs no save, and preserves
  canonical generation 3 plus recovery generation 2 as an exact chain. Journal
  and proof-carrier bytes remain unchanged, the old sentinel stays absent,
  player/commander identity remains exact, overlap is rejected without
  mutation, and cleanup is zero.
...
JIP/reconnect, multi-writer behavior, off-device recovery, performance, soak,
and unrelated Campaign Debug failures remain open.

## Preceding Durable Field-Vehicle Fresh-Process Checkpoint

The ordinary campaign-persistence runner now carries one exact durable field-
vehicle fixture through all five fresh processes. Prepare creates two distinct
S1203 campaign rows with stable IDs and abstract cargo counts 3 and 7. The next
process restores two roots into a fresh Partisan world, moves vehicle A, and
destroys vehicle B through the engine damage path. Capture must retain A's new
full 3D position, normalized upright yaw, and cargo, convert B to an inactive
tombstone, retain B's
cargo relation, and remove the destroyed physical root.

Every later process requires exactly one live A binding and one inactive B row.
Restore may not borrow stale native entity authority: all post-prepare stages
report `adopted=0`, `retired-native=0`, only the expected spawned-root count,
and zero native-tracked durable roots. The shutdown process also requires its
one live root to remain at the saved transform with controller/engine stopped,
supported brakes applied, and hierarchical dynamic physics inactive through
the blocking save callback. Native and profile-fallback no-save verification
must independently reproduce the same serialized A position/yaw, B tombstone,
and cargo counts, with physical placement accepted only inside the proof's
tolerance.

Production enforces the same boundary. The durable vehicle tracker detaches
native entity tracking at initial track and every capture. Restore validates
the complete logical and binding graph and blocks campaign bootstrap until its
detailed receipt is `AllExact`; a bounded timeout fails closed. Destroyed-root
cleanup and legacy tombstone retirement refuse living player occupants. Legacy
retirement is allowed only for one unoccupied native-tracked candidate with the
same normalized full prefab within 3 meters and 3 degrees; ambiguity is fatal.

The final strict chain passes all five stages with exit code `0`:

1. Production periodic `AUTO` at tick 1,802 and
   60.018852233886719 seconds; the repeated dirty mark at
   30.016357421875 seconds does not move the first-edge deadline.
2. Typed `MANUAL` restore/move/engine-destroy/capture with two restored roots,
   then one live row plus one tombstone.
3. Real controlled-end blocking `SHUTDOWN`, with exactly one live root held
   controller/physics-quiesced through commit.
4. Native-source no-save verification of the exact serialized one-live/one-
   tombstone position/yaw/cargo graph plus tolerance-verified physical placement.
5. Profile-fallback no-save verification of that same graph.

All owned cleanup counters return to zero. Foundation validation passes 839
references. The stamped Workbench compile loads 5,837 files and 11,850 classes
at CRC `37604e5a`, reports zero script errors, and leaves zero residue.

This evidence is intentionally narrow. It does not prove fuel, partial damage,
attachments, or physical trunk contents, and its duplicate census is limited to
the expected fixture positions. Arbitrary durable vehicles, Workshop/live
clients, multiplayer/JIP/reconnect, performance, and soak remain open. The
current section above closes the single-overwrite profile mirror gap with a
verified two-generation journal; it does not make that journal atomic.
...
session saves. The serializer requires a versioned envelope, payload presence,
and an exact serialized fingerprint; an unarmed proxy or invalid envelope fails
instead of producing an apparently valid save with no campaign row. Both
mission headers select the persistence world-system configuration and enable
all intended save types.

Startup now waits for the persistence system to become active before choosing
campaign authority. A valid loaded native row wins before the profile fallback.
...
local persistence backend. The packed tree reports build `a6e9069f29f8`.
Prepare selects `new_campaign`, succeeds, commits an exact native save, then
writes a deliberately conflicting current-schema profile fallback. Recovery
selects `native`, succeeds, performs the one expected continuation and owner
transition, commits the native successor save, and leaves the conflicting
fallback unchanged. Replay selects `native` from that successor, succeeds with
no continuation, makes no semantic change, commits no save, and again leaves
the conflicting fallback unchanged. The native UUID handoffs and semantic
fingerprint chain remain exact.

Final stamped Foundation passes at 828 script-symbol references. Final stamped
Workbench compiles 5,834 files and 11,839 classes at CRC `5fdd016f`, reports
script validation successful with zero hard or script errors, and returns all
cleanup counters to zero. The final stamped guarded pack and every native
prepare/recover/replay stage exit `0`. All owned-process, guard, watched-root,
spill, and workspace cleanup counters are zero, including
`WorkspacePackScratchRemaining=0`.
Broader Workshop-package execution, live server-client equality, multiplayer/
JIP/reconnect, performance, and soak remain open.

## Preceding Counterattack Owner-Applied Restart Proof

Production restore now runs generic ownership normalization, then exact
counterattack claimant correlation, before runtime ownership reconciliation.
Rows are correlated once by canonical request ID or exact operation source ID
and must match the full ownership receipt fingerprint plus one legal lifecycle
window.

The focused report proves legal stable pending, materializing pending,
dematerializing pending, and returning completed authority. Negative cases cover
premature, engaged, foreign, distinct duplicate, and source-only orphan rows.
Invalid authority is quarantined through the canonical ownership transition,
zone, and marker graph; the exact order is quarantined at `-69`; and the
operation/manifest/batch/group evidence is held without capture, refund,
settlement, cancellation, deletion, or outcome fabrication. A completed exact
receipt may remain historical evidence after durable outcome and later zone
recapture; incomplete authority may not.

The eighth `owner_applied_pending` cut stages a legal raw owner-applied
incomplete receipt and reciprocal zone backlinks. Persistence restore derives
the normalized pending source before startup ownership reconciliation. The
dedicated ownership-startup flag is false during prepare, true during recovery,
and false during replay. Recovery completes the receipt exactly once, a second
ownership reconciliation is a semantic no-op, and exactly one production
`TickOrder` advances the operation through raw `RETURNING` into normalized
persisted `RETURNING`. Raw and restored returning structures differ only by
process-local normalization and retain the same semantic fingerprint.

Replay consumes the existing canonical recovered carrier without rewriting it.
Canonical overwrite is denied, and the save's SHA-256, byte length, and UTC
last-write identity remain unchanged. Its stamped fingerprint chain is
`6195e5e4f45501ff -> 2f2d2b05b273ec5e -> same`.

Campaign Debug now registers
`enemy_counterattack.ownership_correlation` independently of the aggregate.
The exact command-line suite remains in its already-loaded addon context instead
of taking the base-only scenario transition that previously dropped the HST test
type. The guarded runner proves exact native arguments, exact non-skipped JUnit
identity, one empty failed-list artifact, required correlation evidence,
kill-on-close process containment, recursive reparse-rejecting external census,
sanitized diagnostics, and zero cleanup residue.

Final stamped Foundation passes 819; Workbench passes 5,832/11,835 at CRC
`417e9910` with script validation true and zero hard errors; and all eight chains
pass 24/24 fresh-process stages with exact fingerprints, zero exits, and every
cleanup/watch/spill counter zero. The two stock `SCR_FilterCategory` constructor
diagnostics remain recoverable engine noise and are not represented as an
exception-free run.

## Preceding Exact Counterattack Endpoint Owner/Claimant Restart Proof

The implementation checkpoint retains the seven movement/settlement restart
cuts and adds exact endpoint owner/revision plus ownership-transition claimant
authority. Both carrier families freeze source and target owner faction and
ownership revision. Every cut requires unique endpoint rows and exactly zero
claimants correlated by canonical request ID or operation source ID. Tamper
proof mutates the source revision and injects both claimant identities through
the complete validators. The physical cut inherits the movement-family negative
path and exercises positive endpoint validation on its genuine native graph.

Final stamped
Foundation passes at 819 script-symbol references. Workbench compiles 5,832 Game
files and 11,835 classes at CRC `3131538f`, exits `0`, reports
`ScriptValidation true` and zero errors, and completes exact guarded cleanup.
The final stamped seven-cut process matrix also passes in full: all 21 stages run
build `008cd481d5e5`, exit `0`, preserve exact fingerprint continuity, and leave
all cleanup counters at zero.

Persistence now rejects any open exact defensive QRF, counterattack, or garrison-
rebuild operation in `DEMATERIALIZING` before civilian reconciliation, spawn-
adapter mutation, or any other capture preparation. Existing valid Schema-70
dematerializing saves remain compatible: restore still converts their durable
casualty roster and live cursor once into held `VIRTUAL`/strategic authority.
New captures do not publish an in-progress dematerialization.

Open `PHYSICAL` enemy responses use a two-phase position transaction. The first
phase read-only preflights each reciprocal family graph, requires `LIVE` position
authority, one successful non-held batch, a durable living roster, and complete
adapter/PhysicalWar bindings, then samples the native living centroid without
changing durable position state. After the existing roster and unrelated
physical validators succeed, the apply phase writes the sampled group position
and invokes the family-owned production position transition. If any response
fails, rollback restores every already-applied group position plus operation
strategic position, revision, and last-progress second. Campaign-debug isolated
capture also returns `null` when preparation fails instead of exposing a stale
previous isolated snapshot.

Seven fresh-process `prepare`/`recover`/`replay` chains pass on the final stamped
implementation, with every stage exiting `0`:
...
The dematerializing and materializing cuts now both retain a safe canonical
`VIRTUAL` baseline, exercise the raw interrupted state only on a migration-free
clone, and require production capture to return `null` without changing either
the raw clone or the canonical persisted fingerprint. Their compatibility
normalizers remain covered, but the proof no longer treats raw
`DEMATERIALIZING` or `MATERIALIZING` as a successful checkpoint.

The physical cut is different. It frame-drives native group and member spawning,
handoff, and exact adapter/PhysicalWar registration, proves one live root and the
expected living member bindings, and samples the native centroid as an
independent oracle. It then deliberately makes the durable group and operation
positions stale. Production persistence must refresh both from the native
centroid, independently resolve the centroid again after capture, serialize the
same position, and normalize the graph to held `VIRTUAL`/strategic authority
with one reprojection. Recovery advances the replanned route exactly 75 meters;
replay is a semantic no-op. The guarded profile-fallback file is proof transport,
not certification of normal persistence-source precedence.

The three settlement cuts share one nondegenerate N-1 roster, one charged
resource pool, and `route_failed_survivors`. Their prepared prefixes contain,
respectively, no refund/no receipt; one refund mutation/no receipt; and one
refund mutation plus its durable receipt while the operation remains PREPARED.
The first startup preserves one original debit and one proportional refund,
settles the operation, aborts the terminal order, and removes the batch/group
claimants exactly once. An explicit second reconcile and the replay startup are
both no-ops. Their expected terminal operation revisions are prefix `+2`, `+2`,
and `+1`.

The final source makes carrier-family authority an exact exclusive choice.
Movement cuts require a movement expectation and no settlement expectation;
settlement cuts require the inverse. The launcher now forges a settlement
carrier with both families as a negative self-test and requires rejection. The
final stamped matrix includes this hardening.

All seven final stamped chains require exact carrier/readback fingerprints and
zero owned process, guard, profile, log, spill, temporary-root, mutex, and
cleanup residue. The physical prepare stage additionally requires zero adapter
and PhysicalWar claimants after bounded cleanup; settlement cuts require zero
physical authority throughout. The independent final census confirms zero
engine processes, zero Workbench guard roots, zero restart guard roots, and both
proof mutexes free.

The shared QRF and garrison-rebuild wrappers, process-field clearing, held-roster
normalization, and position alignment have static Foundation coverage only.
This checkpoint does not claim a fresh native physical restart for those two
families, uniform cross-family resource-ledger parity, package/live server-client
execution, networking, migration breadth, marker runtime, performance, or soak.
The current proof above implements the lifecycle-aware pre-reconcile correlation
and quarantine decision that was open here. This preceding proof required zero
such claimants and did not fix production correlation. Durable endpoint ABA
snapshots remain a separate future contract-2 schema decision whose schema
number is not yet assigned. Native persistence-
source selection, world scope, package/live server-client, and network proof
remain open. The full Campaign Debug suite remains uncertified; totals below are
historical evidence rather than results from this focused checkpoint.

## Preceding Phase 24 Runtime-Owner Snapshot Audit Boundary

Campaign Schema 70 and runtime-settings Schema 24 remain unchanged. That
checkpoint was schema-neutral and settings-neutral. Phase 24 preserves its
...
deterministic exact-counterattack lifecycle proof.

Full guarded run `seed1985_t0_p1_u1784134163` on that stamped source ran
from campaign second 0 through 7,201. It passed both Phase-24 assertions: all 14
sampled runtime owners were classified with zero snapshot invariant failures,
and three open exact counterattacks owned three `VIRTUAL` projections with zero
invalid-authority rows and zero support leaks. All 11 Phase-17 assertions and
all six staged spawn-adapter cleanup cases passed; final orphan cleanup was
zero. All 18 state-diff rows were zero, the script/Partisan/crash census was
zero, and every process, guarded-profile, watched-root, and external-spill
cleanup counter was zero. Workbench validation of the same stamped source
passed 5,830 files and 11,822 classes at CRC `e836e3b4`, with script validation
successful and zero hard errors.

The full report contained 687 cases at 583 PASS, 50 WARN, 46 FAIL, 7 BLOCKED,
and 1 SKIPPED. Certification remains open: 5,537 of 5,685 required assertions
were proven, with 134 failed and 14 blocked. This run closes the Phase-24 owner-
snapshot correction and its cleanup gate; it does not certify unrelated open
features.
...
failed to clean an independently complete fixture. That start case
intentionally leaves its exact active group, frozen manifest, and queued spawn
batch for the following proof cases. The generic post-case ownership probe was
introduced before this multi-step proof and therefore knew only production
zone, mission, support, order, and QRF owners. Its one-case lifetime assumption
...
manifest, batch, spawn-result, request, operation, force, and projection
identities must be reciprocal; the manifest must be frozen and its content hash
must recompute exactly; and the batch must carry the same manifest hash. An
arbitrary spawn-adapter row, a partially matching fixture, or any row observed
...
The first Workbench compile of the native casualty-continuity extension exposed
four ordinary `Formula too complex` diagnostics in large proof predicates. The
correction split those predicates into bounded topology, tombstone, resource,
and continuity checks without relaxing their combined result. The corrected
stamped compile/create gate passes as recorded below; this was an Enforce
expression-complexity limit, not another native Workbench crash.
...
settings Schema 24 remain unchanged. It begins from the admitted outbound
`VIRTUAL` aggregate, moves a living controlled player inside the production
materialization radius, invokes the exact counterattack runtime owner to enter
`MATERIALIZING`, advances one projection-scoped production spawn-worker tick per
runner invocation, confirms complete `PHYSICAL` authority, then moves the player
outside the fold radius and returns the same survivors to held `VIRTUAL`
authority. The case does not create a replacement order, manifest, batch, group,
or legacy support request.

Six registered assertions cover the exact baseline, `MATERIALIZING`,
`PHYSICAL`, fold, continuity, and campaign-clock isolation boundaries. Identity
continuity includes the order, operation, frozen manifest, spawn result, force,
and projection; the living-slot fingerprint; original one-pool debit and pool
balances; and zero legacy support rows. The native driver waits for the real
monotonic campaign clock when a retry is not yet due and uses a bounded
real-frame handoff window. It does not write a synthetic future retry second to
shared campaign state. Adapter counters/cursors are restored as part of fixture
isolation.

The current source extends that same production-owned aggregate after the
unchanged R27 cycle. It rematerializes the full roster, resolves one exact
living slot through projection-scoped adapter authority, and applies one real
`SCR_DamageManagerComponent.Kill` engine death. Generic PhysicalWar survivor
sampling excludes exact counterattack groups, so it cannot race the exact slot
ledger; the force-spawn adapter remains the sole casualty owner. A scoped
reconciliation retires exactly one slot, reduces the durable and runtime roster
from N to N-1, detaches the dead entity from adapter and PhysicalWar ownership,
and must become a no-op when replayed.

Exact force-spawn life observation now requires both the controller-aware
living predicate and `SCR_AIDamageHandling.IsAlive`. A controller can still
report its cached living state after the stock damage manager has reached
`DESTROYED`, so either predicate alone is insufficient for exact casualty
authority. The death preflight also requires an enabled damage manager. After
issuing `Kill` exactly once, the fixture yields for at most four real frames and
records damage-state, controller-state, and combined-alive diagnostics. It never
re-kills the entity and never reconciles a slot while either alive predicate is
still true.

Before fold, the retired tombstone retains its last physical entity ID as death
evidence. Production dematerialization then clears that process-local identity
while preserving `RETIRED`, ever-alive, and casualty-confirmed authority. The
N-1 roster re-enters native `PHYSICAL` authority with no handle or entity for
the retired slot, replays reconciliation as a no-op, and folds again without
changing the survivor fingerprint, original debit, pool balances, ownership,
or legacy-support count. The detached proof corpse is deleted explicitly on
both successful and failed fixture cleanup so the debug path cannot leak world
entities.

Projection telemetry now proves phase-specific topology rather than trusting an
enum or aggregate alive count. All four active-group roster counters must match
the authoritative living slots. Partial `MATERIALIZING` and
`DEMATERIALIZING` bindings validate result, request, force, projection,
generation, and slot-kind keys; unique entity objects and IDs; the exact root
registered with PhysicalWar; and every member entity registered to that root.
Members cannot exist without the root, and adapter handle cardinality must equal
root plus members. Idle virtual/materializing authority has no native group ID,
root, member, or handle. Complete physical authority has one root, one member per
durable living slot, and one corresponding root-plus-member handle set.

Failure containment is typed-first. The runner first asks the exact
counterattack owner to terminalize and retire the aggregate. If that owner is no
longer available, the debug-only focal path retires process-local PhysicalWar
ownership by deterministic group ID and adapter handles matching either the
projection identity or spawn-result identity before removing the focal durable
batch/group rows. Cleanup succeeds only when both handle-key counts and all
PhysicalWar root/member ownership are zero. If the tracked exact order is absent
on the first projection frame or disappears later, the run records a fatal
invariant FAIL, performs authority-free containment, stops before Phase 24,
restores the isolated state, and saves `aborted/fatal invariant` artifacts.

R29 exposed a separate physical-confirmation race. The production coordinator
can complete a batch as `PHYSICAL` before the proof's next scoped worker call;
therefore a false next-frame `tickChanged` means only that the current call made
no mutation, not that production authority failed. Initial, casualty, and
survivor confirmation now re-sample the production owner for at most four real
frames and OR-latch mutation telemetry. Success is based on actual authority:
`PHYSICAL` projection with a `LIVE` group, a successful non-held batch, one
unique spawned root, exact durable/runtime/handle/group counts and reciprocal
bindings, and zero legacy support ownership. Once that topology is present the
probe does not requeue the batch, respawn the roster, or call another worker;
terminal contradictions fail immediately and an incomplete topology fails at
the bounded timeout with safe read-only diagnostics.

R30 exposed the next debug-only scheduling seam after a successful native
handoff. The campaign-debug runner advances on a one-second cadence; returning
after handoff allowed ordinary production work to run before the next proof
stage. On the second handoff, the batch was successful and physical cardinality
was exact, yet one present, registered, handed-off member was already nonliving
before the fixture issued its controlled `Kill`. Cardinality therefore did not
prove liveness. The current proof chains a successful handoff through
`PHYSICAL` confirmation and then immediately into the controlled fold or kill
within the same runner invocation. That ordering matches the synchronous
production handoff without consuming an asynchronous settle sample in advance.
Actual death observation, corpse cleanup, fold, and later re-entry still yield
across normal frames. Binding diagnostics now distinguish an incomplete binding
from a complete binding whose entity is nonliving.

Phase 24 escalation telemetry classifies every new order by its production
runtime owner before applying owner-specific expectations. Legacy support
orders are counted separately from exact QRF, counterattack, patrol, rebuild,
quarantined, and unsupported owners. Exact counterattack telemetry uses read-
only production validators for open authority and claimant-free terminal
ledgers, counts `VIRTUAL`/`MATERIALIZING`/`PHYSICAL`/`DEMATERIALIZING`
independently, applies the phase-specific topology above, and treats any legacy
support backlink as an authority leak.

The preceding proof-ordering source is implementation
`393733cc165b96ec494c72f96741cf993d400ebd`, stamp commit `a8210af`, UTC
`2026-07-15T10:46:59Z`, label
`schema70-settings24-native-counterattack-proof-ordering`. Campaign Schema 70
and runtime settings Schema 24 are unchanged. The final stamped tree passes
Foundation at 808 script-symbol references. Workbench Game-module validation
loads 5,830 Game files and 11,822 classes with 47,077K static storage at CRC
`b789ee05`, reports `Script validation successful`, exits `0`, records zero
...
R31 `seed1985_t0_p1_u1784117364` executed that exact source and label through
...
entries. These runtime results close the preceding proof-ordering rerun only;
they
...
native-projection assertions (baseline, `MATERIALIZING`, `PHYSICAL`, fold,
continuity, and clock isolation) passes exactly once. Both Phase 24 production-
owner assertions, `phase24.escalation.runtime_owner_classification` and
`phase24.escalation.exact_counterattack_authority`, also pass exactly once. All
29 final state-diff lines are zero. The wider R27 suite remains diagnostic and
uncertified; its complete totals and still-open failures are recorded below.

R28 `seed1985_t0_p1_u1784098447` ran the first casualty-continuity stamp across
687 cases at 576 PASS, 50 WARN, 54 FAIL, 6 BLOCKED, and 1 SKIPPED. Certification
proved 5,517 of 5,684 required assertions, with 149 failed, 18 blocked, zero
warning, and overall result `false`. The preceding six Phase 17 assertions
passed, but all five casualty assertions failed. Its 29-line state diff and 18
reported deltas were zero, and guarded cleanup was exact.

The unchanged rerun R28b `seed1985_t0_p1_u1784100187` demonstrates that this
was intermittent rather than a deterministic roster defect. Across 687 cases it
recorded 577 PASS, 50 WARN, 53 FAIL, 6 BLOCKED, and 1 SKIPPED and proved 5,523
of 5,684 required assertions, with 143 failed, 18 blocked, zero warning, and
overall result `false`. All eleven Phase 17 assertions passed exactly once. The
casualty path proved N=9 to N-1=8 through death, fold, survivor re-entry, replay,
and final fold. Phase 24 runtime-owner classification passed; exact-counterattack
authority was skipped in that later phase because the isolated Phase 17 owner
had already been retired. The final diff and cleanup were exact.

R29 `seed1985_t0_p1_u1784101849` ran the intervening casualty-settle build
`b6395475b664b513ed371a7a97a13508722a438d`. It recorded 687 cases at 576
PASS, 50 WARN, 54 FAIL, 6 BLOCKED, and 1 SKIPPED and proved 5,521 of 5,687
required assertions, with 148 failed, 18 blocked, zero warning, and overall
result `false`. The first six Phase 17 assertions passed, but the five casualty
assertions failed before `Kill`: rematerialization observed materializing/
successful/physical as `1/1/0`, with death-settle `0/4`. Both Phase 24 assertions
  passed. Their diagnostics reported owners `16/16`, orders/open/terminal/invalid
`1/1/0/0`, projections V/M/P/D `1/0/0/0`, and zero legacy support. All 29 state-
diff lines and 18 reported deltas were zero, no mission remained, the process
exited `0`, and guarded cleanup was exact. This evidence is the reason current
confirmation treats next-frame mutation telemetry separately from already-
completed production authority.

R30 `seed1985_t0_p1_u1784110353` executed 687 cases at 576 PASS, 50 WARN, 53
FAIL, 7 BLOCKED, and 1 SKIPPED. Certification proved 5,510 of 5,672 required
assertions, with 144 failed and 18 blocked. The first six Phase 17 assertions
passed, while all five casualty assertions failed before `Kill`. The second
handoff reported batch succeeded/held/handoffs `1/0/2`, durable living/runtime
members/handles `9/9/10`, and all four group roster counters `9/9/9/9`.
Nevertheless, exact binding validation was false because one present,
handed-off, registered member entity was nonliving. Initial physical settle took
one sample; casualty-re-entry physical settle exhausted four. Phase 24 runtime-
...
The preceding exact-QRF source is stamped at implementation
...
settings Schema 24 are unchanged. This checkpoint closes the exact enemy
defensive-QRF canonical-fallback process-restart proof across three committed
terminal cuts: before refund, after refund, and after receipt.

Each cut uses three fresh processes. `prepare` writes and reads back the exact
`PREPARED` canonical snapshot, `recover` boots from that serialized source and
requires one startup reconciliation to finish the terminal tail, and `replay`
boots from the terminal snapshot and requires startup reconciliation to remain
a no-op. Recovery and replay also invoke a second same-state reconciliation and
compare the independently serialized operation, order, pool balances, mutation
tail, refund receipt, survivor authority, and stable fingerprint. Every
prepare/recover/replay stage passed with exit `0` for all three cuts.

The proof is fail-closed and narrowly armed. An exact `HST_Dev` launch, stage,
run ID, cut, build/schema identity, and explicit canonical-overwrite guard must
all agree before the boot path may mutate profile state. The coordinator then
observes and validates the canonical serialized source, adopts that exact state
before the startup QRF reconciler, captures whether startup reconciliation
changed it, and writes the result artifact last only after source, invariant,
same-state no-op, and persisted-readback checks pass. Process ownership and
cleanup guards apply on both success and failure; the completed matrix left no
engine processes or disposable proof roots.
...
stock player-audit VM diagnostic remains environmental evidence rather than an
HST failure.

The preceding prepared-recovery implementation
`78db295a02936aa66899203cb33e50462b5fd557`, label
`schema70-settings24-exact-qrf-prepared-recovery`, established the terminal
state machine retained by this checkpoint. Exact handling persists `PREPARED`
intent before refund, stages the complete resource tuple with `applied=false`,
validates the original debit, every claimant, and durable survivor authority,
applies or replays the deterministic refund, publishes the applied receipt
last, and only then finalizes the operation and order tail. Current-provenance
`SETTLED` rows still fail closed on a damaged pool tail; arbitrary old partial
rows are not guessed into valid receipts.

R26 `seed1985_t0_p1_u1784074264` remains the in-memory Full Campaign Debug proof
for both exact-QRF assertions and its capture/restore matrix. Its
`persistence.real_restart` case remains BLOCKED because R26 itself does not stop
and relaunch the executable. The new external matrix closes only the guarded
exact-QRF canonical-fallback recovery slice; it does not convert R26 into a
process-restart run or certify native persistence-source selection.
...
`3ded248a4ded084dfb0e3aa8e54ae0a47d36cd5f`, label
`schema70-settings24-debug-cleanup-ownership`, and checkpoint-evidence source
`2508a735863c153f95bae94adb13f3037b4cdeef`, UTC
`2026-07-14T16:57:02Z`, label
`schema70-settings24-debug-checkpoint-evidence`, distinguishes exact isolated-
capture evidence from a production checkpoint request. R21 runtime-proves both
earlier source corrections.

The persistence-roundtrip correction remains implementation
`89b7754bcd9ac7e8c41f2a8d7604784b5c1c1c83`, label
`schema70-settings24-current-support-roundtrip`. It also changed no persisted
schema.
...
R19 `seed1985_t0_p1_u1784044976` first proved every internal persistence
assertion PASS. R23 `seed1985_t0_p1_u1784054690` preserves the seeded roundtrip:
live/restored summary and report are exact; counts match at 11/11
missions, 22/22 assets, 21/21 runtime entities, 9/9 groups, 10/10 runtime
vehicles, and 1/1 field vehicles. `civilian_occupier_support` remains 2,514 on
both sides. Only `persistence.real_restart` remains BLOCKED, as expected for an
in-process roundtrip that does not restart the executable. Later phase-level
persistence smoke/report cases WARN after subsequent run drift; they do not
invalidate the earlier exact seeded capture/restore result.

## Current Spawn-Queue Resume Boundary

The production queue selects a bounded winner before
`StartAttemptIfReady()` normalizes the next attempt. The previous selector's
`HasSpawnWork()` recognized only `QUEUED` slots, making a
`FAILED_RETRYABLE` or `DEFERRED` slot invisible before it could reach the
normalizer. That single gate defect prevented retry generation advance,
same-wave progression, and restored nonterminal replay. Stale-generation and
no-duplicate assertions consequently had no retry callback to evaluate.

Current selection treats `DEFERRED` and `FAILED_RETRYABLE` slots as selectable
only while the owning batch is `PENDING`, `DEFERRED`, or `FAILED_RETRYABLE`.
The manifest must remain exact, dependencies must still be registered and
ready, and registered siblings remain preserved. Selection itself stays
read-only; generation advance and slot normalization still occur only after
bounded selection.

R22 passes all 18 `early_mechanics.spawn_queue` assertions. The retry advances
generation 1 -> 2 at second 103, selects scheduled work and only the failed slot,
rejects the stale callback, completes and replays exactly once, and retains four
registered slots. Same-wave exact retry, sibling preservation, and terminal
progression pass. Interrupted restore advances generation 1 -> 2 at reconciled
sequence 9, changes on the first reconciliation, is a no-op on repeat, and
completes once. This is deterministic production-service evidence; native
physical registration, packaged restart, multiplayer, and soak remain separate.

## Current Active Demolition Witness Boundary

R22 proved that resource-name and component hints alone were not physical
explosive evidence: worn grenade equipment, carried ammunition, and an equipped
launcher could all sit inside the 45-metre scan and contribute destroy score.
Current source first requires an unparented entity with a projectile component,
rejects any inventory item occupying a parent slot, and then requires either a
trigger component that has actually triggered or a moving projectile component
with nontrivial velocity. This structural gate precedes resource-text scoring.

Entity-backed callback and proximity observations use one canonical prefab-plus-
entity key. Each component holds at most 64 accepted keys for its lifetime,
rejects duplicate observations, and fails closed when that receipt set is full.
The component records a key and increments its local accepted-source count only
after the authoritative mission asset shows a real damage, hit, or destroyed-
state mutation. The primitive proof now samples the asset before its explicit
damage request and requires `primitive.destroy.no_ambient_witness_score` to show
zero ambient demolition damage, hits, evidence, and destroyed state.

R23 proves this boundary on implementation
`0e54f6cbc7f7084e5534fc603b491cba0d91b653`. All six generic destroy-target
primitive cases pass `primitive.destroy.no_ambient_witness_score` before the
explicit damage request with damage 0, hits 0, source none, evidence 0, and
destroyed 0. The exact radio-tower primitive and those six generic destroy
primitives all pass their mission-start, mission-runtime, and primitive-runtime
cases. Their cleanup cases are WARN-only evidence because each mission had
already completed through its runtime path. The 45-metre fallback is still
proximity evidence rather than collision proof, and native callback source
identity remains a future tightening once it is wired reliably.
...
During a state-isolated Campaign Debug run, the coordinator holds its ordinary
per-frame enemy-commander cadence and ambient local-security producer. Phase 18,
Phase 22, and Phase 24 fixtures still invoke the production commander `Tick()`
path explicitly, and the detached local-security proof continues through its own
state and service. The Phase 18 entry guard also fails closed if an untracked
open Petros order or a new active Defend Petros mission exists. This prevents a
scheduler frame between fixtures from creating unowned authority while
preserving explicit production-path proof.

R19 exposed why the local-security producer must be held with its process
worker. Debug isolation suspended force-spawn queue execution, but an ordinary
local-security tick released Morton's held patrol into `MATERIALIZING`. The
matching worker could not complete that transient state, so persistence
correctly deferred rather than capture ambiguous casualty authority. Build
`94415c8` holds that ambient producer only while Campaign Debug owns the isolated
clone; production campaigns are unchanged. R20 first removed the deferral, and
R21 confirms no local-security materialization deferral with all eight
`local_security` assertions PASS.
...
Compared only at the aggregate level, the retained later-package run adds one
...
assertions passed: expected/classified owners are 14/14 with zero snapshot
invariant failures, and three open exact counterattacks own three `VIRTUAL`
projections with zero invalid authority, zero invalid rows, and zero support
leaks. All six staged spawn-adapter cleanup cases passed, and final orphan
cleanup was zero.

All 18 reported state-diff rows are zero. All three artifacts remained stable,
non-empty, and parseable, and the run records zero script, Partisan, or crash
errors. The instrumented runtime completed in 815 seconds. Its guarded cleanup
left every process, guarded-profile, watched-root, external-spill, deletion,
modification, missing-root, and cleanup-error counter at zero. This proves the
Phase-17 lifecycle, Phase-24 sampled-owner correction, isolated state
restoration, and exact external cleanup boundary for this run. It does not
convert the remaining 46 FAIL, 7 BLOCKED, 50 WARN, and 1 SKIPPED results into
passes or close package, process-restart, multiplayer, or soak gates.
...
failed and 18 blocked. Its first six Phase 17 assertions passed while all five
casualty assertions failed before `Kill` at materializing/successful/physical
`1/1/0`. Both Phase 24 owner assertions passed, all 18 reported deltas were
zero, and cleanup was exact. R29 established that mutation telemetry could lag
already-completed production authority; R30 then narrowed the remaining defect
to once-per-second proof scheduling after the second handoff.

R28 `seed1985_t0_p1_u1784098447` is the first run of the casualty-continuity
stamp. It executed 687 cases at 576 PASS/50 WARN/54 FAIL/6 BLOCKED/1 SKIPPED and
proved 5,517/5,684 required assertions, with 149 failed and 18 blocked. Its six
preceding Phase 17 assertions passed while all five casualty assertions failed;
the 29-line state diff and all 18 deltas were zero and cleanup was exact.

R28b `seed1985_t0_p1_u1784100187` reran that stamp unchanged. It executed 687
cases at 577 PASS/50 WARN/53 FAIL/6 BLOCKED/1 SKIPPED and proved 5,523/5,684
required assertions, with 143 failed and 18 blocked. All eleven Phase 17
assertions passed exactly once and proved a real N=9 to N-1=8 death, fold,
survivor re-entry, replay, and final fold. Phase 24 runtime-owner classification
passed and exact-counterattack authority was skipped after Phase 17 retired the
isolated owner. The final diff and guarded cleanup were exact. The different R28
and R28b outcomes on identical source established an intermittent handoff race,
which R29 narrowed to physical confirmation rather than casualty bookkeeping.

R27 `seed1985_t0_p1_u1784093667` remains the preceding projection-only proof. It
executed 688 cases: 577 PASS, 49 WARN, 55 FAIL, 6 BLOCKED, and 1 SKIPPED and
proved 5,500 of 5,663 required assertions, with 145 failed and 18 blocked. Its
six Phase 17 projection assertions and both Phase 24 owner assertions passed
exactly once, and all 29 final state-diff lines were zero. Those historical
results do not reclassify any later failure or certify route travel, combat,
capture, return, settlement, restart, package, network, JIP/reconnect, or soak.

R26 `seed1985_t0_p1_u1784074264` remains the preceding prepared-QRF comparison.
It executed 688 cases at 577 PASS, 51 WARN, 54 FAIL, and 6 BLOCKED, proving 5,504
of 5,667 required assertions with 145 failed and 18 blocked. It ran build
`78db295a02936aa66899203cb33e50462b5fd557`, label
`schema70-settings24-exact-qrf-prepared-recovery`; both
`enemy_qrf.settlement` and `enemy_qrf.persistence` passed. Its engine log is not
exception-free: startup emits the known base-game player-identity audit/
reconnect/arsenal VM exceptions, and deliberate corruption fixtures emit error-
severity fail-closed diagnostics. Those remain historical R26 evidence, separate
from the R27 projection PASS results and from overall suite certification.

R26 preserves exact support-only and dual-pool debit/refund replay and adds six
committed crash cuts across pre-refund, post-refund, and post-receipt states plus
three uncommitted full-refund cuts. First recovery is exact; same-state replay
and a second capture/restore are no-ops. Corrupted prepared authority, a damaged
pool tail, and a tampered survivor tuple fail closed without another resource
mutation; the bad current-provenance settled tail additionally proves stable
same-state replay and second-restore quarantine. Mutationless history still
restores without invented provenance. A missing required group backlink
continues to produce `ABORTED` / `exact_operation_invalidated`, retain its
authority rows, leave both pools unchanged, and keep the receipt open rather
than fabricating completion.

R25b `seed1985_t0_p1_u1784063032` remains the dated refund-authority comparison
on implementation `434b73a16ae92911896fdec095af6bce88168916`. It executed the
same 688 cases at 577 PASS/51 WARN/53 FAIL/7 BLOCKED, first passed both exact-QRF
assertions under the QRF-specific validator, and restored clean typed cleanup,
but it predates durable prepared crash-cut recovery.

R20 closes the local-security checkpoint blocker from R19. It contains no
`exact local-security roster reconciliation failed` deferral, and all eight
local-security catalog, replay, eligibility, survivor/restore, destruction,
non-loss settlement, epoch-rearm, and conflict-quarantine assertions PASS. Its
foundation checkpoint command also succeeded, refreshed the current timestamp,
and captured a current-schema deployed-HQ snapshot. The remaining
`checkpoint.persistence_status` FAIL is a proof-classifier false negative: the
isolated path correctly recorded `isolated manual checkpoint`, while the
assertion recognized only the production `checkpoint requested:` prefix. Source
checkpoint `2508a73` now branches on exact isolated versus production evidence;
R21 confirms the foundation checkpoint PASS with isolated evidence.

R20's cleanup snapshot still WARNs because open enemy orders changed from 0 to
4. Direct Phase 22 commander ticks could append incidental orders without
registering them with the debug cleanup owner, and the run baseline compared all
historical rows with the final open-row count. Cleanup-ownership checkpoint
`3ded248` registers every appended order through the existing identity-safe
cleanup path and uses open-order counts at both ends. R21 confirms typed enemy cleanup PASS with zero
open orders and the leak snapshot PASS at 0 -> 0. The separate
`isolation.world_scope` assertion remains intentionally BLOCKED because player,
world-runtime, and service-cache restoration requires restarting the disposable
development session.

R22 returned the three `destroy_outpost_cache` cases reported under
`destroy_factory_asset` from R21 WARN to PASS and exposed why they could complete
prematurely: worn explosive equipment could satisfy production destroy scoring
away from the intended physical target. R23 closes that classifier defect. Six
generic destroy primitives each pass the new pre-action ambient-witness
assertion at zero damage, hits, source, evidence, and destroyed state, then pass
their explicit runtime completion path. The separate exact radio-tower destroy
primitive also passes, so all seven destroy-family start, runtime, and primitive
cases are green. Cleanup remains WARN-only because those missions had already
completed through their runtime path.
...
R23's dated final state diff is exactly zero across elapsed time, resources,
training, war level, missions, objectives, runtime vehicles, assets, groups,
support, orders, markers, garage, arsenal, civilian zones, strategic events,
and undercover state. Its seeded in-memory roundtrip also proves exact summary,
report, typed counts, and civilian support. R23 nevertheless left one tracked
QRF settlement failure, one tracked open row, two exact runtime claimants, and
an open-order leak from 0 -> 2. That evidence remains the original reproduction
of the QRF settlement defect rather than the current result.

R24 `seed1985_t0_p1_u1784059798`, build
`6303e5817a924091258c9cf0dbccdd2e0731c1e3`, executed 688 cases at 578 PASS,
50 WARN, 53 FAIL, and 7 BLOCKED. It proved 5,522 of 5,682 required assertions,
with 142 failed and 18 blocked. R24 isolated the remaining QRF failure to the
wrong cross-family one-pool validator, while its proof also used one brittle
failure sentence and bool-shaped terminal/status telemetry. Typed cleanup,
tracked open-order cleanup, the 0 -> 0 leak snapshot, Phase-24 core assertions,
and the final exact-zero tracked-state diff were already clean.

R25b replaces that validator boundary and the brittle proof classification.
Typed cleanup reports settled 0, failures 0, open tracked orders 0, and runtime
claimants 0; the open-order leak remains 0 -> 0. The seeded roundtrip remains
exact at 11/11 missions, 22/22 assets, 21/21 runtime entities, 9/9 groups,
10/10 runtime vehicles, and 1/1 field vehicles. Every final tracked-state delta
is zero. Phase-24 core assertions pass, with escalation physicalization retained
as WARN telemetry rather than a QRF authority failure. At the R25b checkpoint,
durable QRF `PREPARED` recovery across a real process stop was still the next
QRF-specific gap. The separate guarded canonical-fallback process matrix closes
that narrow gap. Packaged server/client execution, native persistence-source
selection, multiplayer/JIP/reconnect, migration, world/physical behavior, and
soak gates still prevent certification.
...
structured artifacts beneath `$profile:Partisan/debug`. R25b confirms that
trigger, all three JSON/summary/state-diff writes, its then-current build label,
and an exact-zero final state diff. R26 later confirms the prepared-recovery
checkpoint, while R27 confirms the preceding stamped native-projection
checkpoint, the same CLI trigger, eight new owner/projection assertions, and a
29-line zero state diff. R28b proves all eleven Phase 17 assertions on the first
casualty stamp; R29 then reproduces the intermittent rematerialization handoff
...
external cleanup remain exact. R31 proves the corrected preceding source: all
...
below. At that checkpoint the full suite had not yet been rerun, so that CLI
artifact remains historical diagnostic evidence rather than passing Schema-70
evidence. Later R25b executes the corrected tree; its dated result and the
later R26 and current R27 comparisons are recorded above.
...
authoritative resource pool, so the top-up was correctly rejected first. At that
checkpoint another full run was required. R25b is the later coordinator rerun:
it preserves the clean seeded/cleanup boundaries and removes the exact-QRF
settlement blocker, but its unrelated failures do not promote the wider suite to
certification. The current section at the top closes only the guarded virtual
delivery-pending JSON restart slice. Physical/materialized continuation,
native-session authority, packaged live gameplay, migration runtime,
...
The sealed R27 tree extends this preceding service-level boundary with
the production-owned Phase 17 `VIRTUAL` -> `MATERIALIZING` -> `PHYSICAL` ->
`VIRTUAL` case and Phase 24 owner-aware exact telemetry summarized above. It
also makes missing-order containment fatal to the isolated run and requires
typed-first, projection-and-result-key cleanup with exact PhysicalWar topology.
Those additions are not part of the older scoped engine checkpoint below. They
are sealed at implementation `4757bc86ffbc7a5fa08e64a9abf7ef74ddc1c003`,
stamp commit `a22e6af`; Foundation and guarded Workbench compile/create gates
pass, and R27 proves all six native-projection plus both Phase 24 owner-aware
assertions once. Broader counterattack runtime gates remain open as listed
below.
...
## Historical Schema-68 Source/Workbench Delivery Boundary

This dated block distinguished the then-current sealed Schema-68 source/
Workbench tree from its previous packaged evidence. Later runtime entries retain
inspected gameplay evidence and must not be read as proof that a newer source
change was executed or certified. The revision diary below is historical
implementation context; the active Schema-70/R26 boundary is recorded above.

At this historical point, work had entered Blueprint Phase 9 of 13 through
sealed Schema 67 and the sealed Schema-68 planning, startup/profile/marker,
commitment-aware planning, and enemy-planning engine-proof checkpoints; this was
not nine completed phases. Blueprint Phase 8 and every
...
### Historical Sealed Schema-68 Enemy-Planning Engine-Proof Checkpoint
...
- The inspected gameplay artifact predates campaign schema 43 exact-force
  ...
  R23 executes all 18 spawn-queue assertions and all 11
  `early_mechanics.spawn_queue_physical_adapter` assertions PASS inside the
  engine process. Packaged native group realization, process restart,
  multiplayer, and soak remain separate gates.
...
- Persistence proof is still in-process/seeded for the one-button runner. The separate guarded chain now has final stamped narrow fresh-process native source-selection evidence, but generic campaign restart, multiclient reconnect/soak, and physical field-vehicle respawn after process restore remain external/manual.
...
- Phase 12 persistence smoke now records a typed seeded-state case. It asserts the seed/run/report PASS status, manual checkpoint evidence, expected-summary task, active smoke missions, convoy/primitive smoke matrices, mission assets/runtime entities, garage cargo, a restore-eligible field-vehicle runtime sentinel, support/enemy-order sentinels, civilian/undercover sentinel records, and an in-memory `HST_CampaignSaveData` capture/restore whose restored summary/report/counts match the live state. R22 proves that seeded roundtrip exactly, including typed counts and `civilian_occupier_support` 2,514/2,514; later phase persistence smoke/report cases WARN after subsequent drift. It explicitly records `persistence.real_restart` as BLOCKED external evidence, not PASS; reconnect remains a separate external proof gate.
...
- Sealed Schema 68 implements persisted per-enemy planning cadence,
  ...
  The dated bootstrap/recovery/throttle correction passed its Foundation and
  all-five Workbench validation, but its Full Campaign Debug coordinator/live-
  authority execution and packaged restart proof remained open.
- The historical Schema-68 enemy-planning engine-proof checkpoint is sealed at
...
- Strategic projection is limited to eleven explicit family consumers across nine
  ...
  migration/archive
  replay remain open.
- Persistence depth is still incomplete beyond the narrow native owner-applied chain: generic campaign restart, multiclient reconnect/soak, and physical field-vehicle respawn after a process restore remain open.
...
- The prior Schema 68/settings 24 planning seal has the implementation identity,
  ...
  historical checkpoints. Its historical engine-proof seal is implementation
  `4c9a94a1cb4811b6e75a7dca5dba70efffcb523d`, UTC
  `2026-07-13T15:43:01Z`, label
  ...
  result for that checkpoint. The later `f97b12e` package exposed fresh
  `-67`/`-68` quarantine and
