...
- [x] Run three fresh packaged dedicated-server processes across
  `prepare -> recover -> replay`, cutting the exact rebuild while it is
  `delivery_pending`.
...
- [ ] Physical/live movement, native-world/entity handoff, multiplayer/JIP,
  and soak remain open. This closes the exact JSON fresh-process restart
  subgate, not broad runtime parity.
...
The final stamped proof evidence is clean. Foundation passes all 865
script-symbol references. Workbench loads 5,846 files and 11,876 classes at CRC
`57609980` with no HST, script, or hard errors and zero cleanup residue. The
...
fallback restores are read-only, exact field-vehicle authority is preserved,
and cleanup is zero. The separate native-versus-stale-journal counterattack
...
| --- | --- | --- | --- | --- | --- |
| CRI-0 Truth and baseline | Campaign Schema 71/runtime-settings Schema 24 is current; earlier Schema-70 and counterattack restart checkpoints remain historical evidence | Current implementation/source is `402b3531a5a150dba51f6063b6936c76dd6db682`, UTC `2026-07-17T18:26:37Z`, label `schema71-settings24-garrison-rebuild-restart`; it retains the two-generation recovery journal, monotonic native/journal order, and journal-authoritative administrative reset while adding the exact rebuild fresh-process restart proof | Foundation passes 865. Workbench passes 5,846/11,876 at CRC `57609980` with zero hard errors and cleanup. The focused journal testcase passes 1/1, 41/41 exact booleans, and native-load v1/v2/bad/future at 1/1/1/1 with an empty failed list. The ordinary five-process chain passes generations 1 -> 2 -> 3, exact canonical generation 3, two valid slots, an exact chain, read-only native/profile-fallback restores, and exact field vehicles. The stale-journal/native chain passes 3/3, selects native, and leaves both journal files unchanged with an exact chain and zero cleanup. The administrative-reset chain passes 3/3, selects the newer generation-3 journal over stale native, performs no final save, preserves the exact two-slot chain and proof carrier, rejects overlap without mutation, and leaves cleanup at zero. The exact rebuild chain passes all three fresh processes with prepared digest `6500277f9189140a`, delivered digest `37daf2da7242f82c`, canonical generation 1, recovery generation 2, byte-read-only replay, and zero cleanup; the separate focused CLI invocation produces no JUnit result and is not a focused PASS | No | Prove broader active-world families, physical/live movement and native-world/entity handoff, Workshop/live clients, arbitrary migration, markers, network/JIP/reconnect, performance, soak, multi-writer/off-device recovery, abrupt termination beyond the last completed checkpoint, and unrelated Campaign Debug failures. |
...
| CRI-2 Force manifests | Schema-70 exact authority plus the Schema-71 fresh-process JSON restart subgate is implemented | Durable SpawnQueue and exact infantry adapters retain the sealed consumers; the garrison-rebuild slice adds one capacity-bounded frozen infantry manifest without widening vehicle, asset, or multi-root admission | Three fresh packaged dedicated-server processes preserve nine accepted slots, one confirmed casualty, and the same eight-slot delivered held roster without refill or aggregate double count. Physical/native live behavior remains unproved | No | Prove the roster through physical/live casualties, virtual/physical transfer, native-world/entity handoff, delivery, re-entry, multiplayer/JIP, and soak while historical contract-zero rebuilds remain isolated. |
| CRI-3 Force runtime | Scoped source/engine proof plus one exact virtual fresh-process restart subgate; runtime certification open | Existing casualty/reprojection paths remain. Schema 70 adds exact garrison-rebuild strategic/physical transfer, casualty fold, delivered held-roster authority, and terminal survivor settlement over one durable roster | The `delivery_pending` chain restores `225/300m`, advances and delivers exactly once, holds eight survivors, and replays as an exact semantic no-op. Production render-bubble and physical behavior are unchanged | No | Package-prove physical/live rebuild movement, casualties, native-world/entity handoff, fold/re-entry, held delivery, ownership invalidation, multiplayer/JIP, and soak alongside every earlier force family. |
| CRI-4 Operation records | Exact garrison-rebuild authority plus the Schema-71 fresh-process JSON restart subgate is implemented | Schema 70 adds a separately typed exact rebuild graph with reciprocal order/operation/manifest/spawn/group identity. Durable PREPARED resource-receipt and SETTLED-before-order-tail crash windows reconcile through production restore without duplicate refund or mutation | Prepare/recover/replay proves exact prepared and delivered fingerprints, once-only delivery and zero-refund receipt/resource mutation, canonical generation 1 to recovery generation 2, byte-read-only replay, and zero cleanup. The focused CLI run has no JUnit result and is not a focused PASS | No | Prove the reciprocal graph through physical/live and native-world/entity handoff, ownership ABA rejection, quarantine idempotency, dedicated-server multiplayer/JIP/reconnect, and soak. |
...
| CRI-9d Exact enemy garrison rebuild | Schema-70 exact authority plus the Schema-71 fresh-process JSON restart subgate is implemented | New contract-`1` rows bind one capacity-bounded frozen infantry manifest, one prepaid support debit, and reciprocal order/operation/manifest/spawn/group authority. Casualties persist across strategic/physical transfer; delivery hands survivors to held garrison authority without aggregate double count. Historical contract-zero rebuilds stay isolated; malformed current graphs quarantine at `-70` | Three fresh packaged dedicated-server processes cut `delivery_pending` at `225/300m` with nine accepted/eight living and one casualty; recover advances and delivers once with eight held survivors and one zero-refund resource receipt; replay is an exact semantic no-op with journal/carrier bytes unchanged. Canonical generation 1 to recovery generation 2 is exact, all exits are `0`, and cleanup is zero. The focused CLI run loses its requested test type and emits no JUnit result, so no focused PASS is claimed | No | Prove physical/live movement, native-world/entity handoff, ownership ABA rejection, quarantine idempotency, multiplayer/JIP/reconnect, and soak. |
...
| Durable operation state | Every force/support/order has an immutable assignment, duty state, engagement mode, materialization state, route progress where applicable, recall/return policy, and terminal result. | Sealed multi-family foundation plus exact rebuild fresh-process JSON restart proof | The exact garrison-rebuild type adds capacity-bounded admission, casualty continuity, held delivery, and PREPARED/SETTLED reconciliation. Its three-process `delivery_pending` recovery/replay subgate closes without duplicate delivery, resource mutation, roster aggregation, or replay writes. Prove physical/live movement, native-world/entity handoff, generalized encounters, vehicle/multi-root policy, multiplayer/JIP, historical missions/garrisons, and other families. | Highest |
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Versioned save state | Durable campaign facts survive restarts and schema migration. | Schema-71 Recovery Journal Implemented / Final Stamped Proofs Passed | One engine-owned proxy carries the campaign DTO. Native envelope v2 and journal envelopes store exact serialized payload strings under UUID-v8/SHA-256 fingerprints and validate bytes before parsing; structured Schema-70 native v1 is reconstructed, validated, and normalized. Typed AUTO/MANUAL/SHUTDOWN requests use one bounded completion owner. Persisted checkpoint/restore/time order reconciles valid native and journal snapshots, with future/ambiguous authority failing closed. Foundation, Workbench, focused authority, ordinary five-process, and stale-journal/native proofs pass; broader active-world serialization and arbitrary migration remain open. | Highest |
...
| Enemy commander orders | Counterattacks, rebuilds, roadblocks, support calls, and HQ pressure queue durable orders. | Exact rebuild fresh-process JSON restart subgate passes / prior exact-counterattack and planning seals retained | The rebuild `delivery_pending` prepare/recover/replay chain preserves its route, casualty, held roster, once-only receipt/resource mutation, and exact replay fingerprint. Planner, physical/live movement, native-world/entity handoff, multiplayer/JIP, and soak gates remain open. | Highest |
...
| Physical response | Near-player enemy responses spawn, move, fight, and fold back. | Schema-70 source/engine proof sealed / Needs runtime proof | Exact garrison-rebuild members materialize from the frozen roster, fold confirmed casualties back to strategic authority, and deliver only surviving held slots. The JSON fresh-process subgate does not prove physical/live spawn, movement, contact, native-world/entity handoff, fold/re-entry, casualty, marker, multiplayer/JIP, or soak behavior. | Highest |
