...
- Server-created dynamic marker prefabs need one valid replication component. Vanilla squad markers declare their own non-spatial/non-streamable `RplComponent`, but a prefab inheriting `MapMarkerEntityBase.et` already inherits an RPL component; do not add a second one. The duplicate-component warning (`SCR_MapMarkerEntity component RplComponent cannot be combined with component RplComponent`) can leave `FindComponent(BaseRplComponent)` null during marker init and make player markers reconcile but fail to render.
...
- Keep live inventory payloads single-owner.
  ...
  - Structural wearable/storage prefab tokens are inconsistent. Treat `fieldpack`, `field_pack`, `field pack`, `ruck*`, and ALICE pack names as backpack/wearable storage aliases, while blocking `pouch`, `carrier`, e-tool `case`/`holder`/`cover` qualifiers, and similar structural child containers from arsenal storage candidates. Do not block the actual equippable entrenching tool just because its name contains `etool` or `entrenching tool`.
...
- `ItemPreviewWidget` does not reliably render every inventory prefab.
  ...
  - Worn clothing Edit should enumerate inspectable `AttachmentSlotComponent` children on the equipped clothing entity and open the normal attachment child-node view. Do not route that button to cargo storage; storage browsing is for adding/removing inventory contents inside the worn item.
