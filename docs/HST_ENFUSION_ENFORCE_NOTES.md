...
- Player-requested support cooldowns can poison later support smoke steps.
  ...
  - If a feature-specific typed case already records the command text as evidence and asserts the command result, do not also emit a generic `action.*` case for the same command. Stage 3 resource award, income tick, training, and player support requests are examples where the typed case owns the command result.
