...
class HST_RuntimeSettingsService
{
	static bool LoadDebugLoggingEnabledQuiet()
	{
		...
		FileHandle file = FileIO.OpenFile(sourcePath, FileMode.READ);
		if (!file)
			return enabled;
	...
	HST_RuntimeSettings LoadOrCreate()
	{
		...
		if (!FileIO.FileExists(sourcePath))
		{
			...
			Print("Partisan | generated first-load settings at " + HST_ProfilePathService.SETTINGS_FILE);
			return settings;
		}
		...
		if (!lines || lines.Count() == 0)
		{
			...
			{
				WriteDefault(settings);
				Print("Partisan | legacy settings file is empty or unreadable; preserved it and generated canonical defaults", LogLevel.WARNING);
				return settings;
			}

			Print("Partisan | settings file is empty or unreadable, using built-in defaults", LogLevel.WARNING);
			return settings;
		}
		...
		if (!LooksLikeJson(lines))
		{
			...
			{
				WriteDefault(settings);
				Print("Partisan | legacy settings file is malformed; preserved it and generated canonical defaults", LogLevel.ERROR);
				return settings;
			}

			Print("Partisan | settings file is malformed, using built-in defaults without overwriting it", LogLevel.ERROR);
			return settings;
		}

		ApplyKnownKeys(settings, lines);
		bool migrated = MigrateSettings(settings);
		settings.Normalize();

		if (HST_ProfilePathService.IsLegacyPath(sourcePath))
		{
			WriteDefault(settings);
			bool adopted = FileIO.FileExists(HST_ProfilePathService.SETTINGS_FILE);

			if (adopted)
				Print("Partisan | adopted legacy runtime settings into " + HST_ProfilePathService.SETTINGS_FILE);
			else
				Print("Partisan | loaded legacy runtime settings but could not create " + HST_ProfilePathService.SETTINGS_FILE, LogLevel.WARNING);
		}
		else if (migrated)
		{
			WriteDefault(settings);
			Print("Partisan | migrated runtime settings to schema " + settings.m_iSchemaVersion);
		}

		Print("Partisan | loaded runtime settings from " + sourcePath);
		return settings;
	}
	...
	protected bool LooksLikeJson(notnull array<string> lines)
	{
		...
		foreach (string line : lines)
		{
			...
			if (line.Contains("}"))
				sawClose = true;
			if (LineHasKeyStatic(line, "schemaVersion"))
				sawSchemaVersion = true;
		}

		return sawOpen && sawClose && sawSchemaVersion;
	}
	...
	protected void WriteDefault(notnull HST_RuntimeSettings settings)
	{
		...
		array<string> lines = {};
		lines.Insert("{");
		lines.Insert("  \"_comment\": \"Generated by Partisan. JSON does not support real comments, so _comment fields explain nearby settings and are ignored by the loader.\",");
		...
		lines.Insert("    \"_comment_debugLoggingEnabled\": \"Enables verbose Partisan diagnostic logging from runtime systems that check this setting.\",");
		...
		lines.Insert("    \"_comment_gameMasterBudgetsEnabled\": \"When false, Partisan disables Game Master placement budget caps at runtime.\",");
		lines.Insert(string.Format("    \"gameMasterBudgetsEnabled\": %1,", JsonBool(settings.m_Features.m_bGameMasterBudgetsEnabled)));
		lines.Insert("    \"_comment_showPlayerMapMarkers\": \"Shows Partisan player-facing campaign markers on the map.\",");
		...
		lines.Insert("  }");
		lines.Insert("}");
		WriteLines(HST_ProfilePathService.SETTINGS_FILE, lines);
	}
