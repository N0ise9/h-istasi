...
- Phase 17 smoke now records typed capture/counterattack assertions for seeded capturable zones, ownership flip, progress reset, starter resistance garrison, debug-prefixed counterattack orders, order costs/positions/status, and marker/report evidence. Physical counterattack advance remains WARN/not covered.
...
- Phase 14-24 smoke steps remain partly legacy string classifications outside the newly typed Phase 14 arsenal, Phase 15 garage/source-vehicle, Phase 16 garrison/training, Phase 17 capture/counterattack state, Phase 18/19 enemy/support, Phase 20/21 civilian/undercover, and Phase 24 pacing/end probes. Counterattack physical advance, HQ threat/Defend Petros, and UI/markers still need stronger typed assertions. Phase 16 proves abstract garrison/training state only; it does not physicalize recruited units. Phase 17 proves counterattack order state only; it does not sample attacker movement. Phase 24 does not yet prove long-window escalation pressure or post-end service inactivity.
...
- Counterattack physical spawn/advance/wave tests are not implemented. Phase 17 now records WARN evidence for this gap instead of treating counterattack order creation as physical proof.
