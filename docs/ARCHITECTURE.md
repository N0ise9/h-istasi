...
The active candidate's package-bound focused set passed under clean harness
...
The native-engine/world rung remains `not-run`. Run the corrected
`force_authority` canary next and the full profile only if that canary accepts.
No evidence from a superseded package can satisfy any rung.
...
sealed in the active `partisan-rc-e11e7ea88a44-20260719T040154Z`. Its exact
package-bound five-case focused set is now accepted as non-certifying
deterministic-service evidence, while its corrected-canary and full-profile
gates remain `not-run`; no historical result can be attached to it.
