...
| Field | Current value |
| --- | --- |
| Status data as of | `2026-07-19T06:40:36Z` |
...
| Deterministic service contracts | `not-run` | The five canonical packaged focused service suites have not run against the active replacement package; historical focused results do not transfer. |
| Native engine-world behavior | `not-run` | No corrected force-authority canary or Full Campaign Debug evidence exists for the active replacement package; native engine-world proof begins only after its packaged focused set is accepted. |
| Packaged dedicated server | `not-run` | The active replacement candidate has not been launched through the standard dedicated-server runtime gate. |
...
| Canary release | `blocked` | Blocked until the active package passes each lower release-proof rung. |
| Stable certification | `blocked` | No current matrix row is certified, and the active package evidence chain has not passed its first runtime gate. |
...
- Active packaged focused autotests: **not run** for replacement candidate `partisan-rc-ee0e8add2a29-20260719T063815Z`; no prior-package result transfers to this package.
- Active Campaign Debug: **not run** for replacement candidate `partisan-rc-ee0e8add2a29-20260719T063815Z`; the corrected canary follows only after the packaged focused set is accepted.
...
| ID | Category | Blocker |
| --- | --- | --- |
| `STATUS-001` | `AUTH` | The active replacement package has green Foundation and all-target Workbench evidence, but its packaged focused set has not run. Deterministic-service and native engine-world proof are not run, and no historical candidate result transfers into this fresh package evidence chain. |
| `STATUS-002` | `TEST` | Run the five individually named packaged focused service suites against the exact active candidate first. Only after that package-bound set is accepted may the corrected force-authority canary and then Full Campaign Debug run against the same unchanged package. |
...
Run the individually named packaged focused service suites next against active replacement `partisan-rc-ee0e8add2a29-20260719T063815Z`, manifest `docs/evidence/release-candidates/partisan-rc-ee0e8add2a29-20260719T063815Z/candidate.json`, and aggregate package SHA-256 `981258439b9d08866c4883471cacfe33aa373a36a667a39e8c939f285db74daf`. If that exact-package set is accepted, run the corrected force-authority canary next; do not transfer the historical candidate's pass or rejection into either gate.
