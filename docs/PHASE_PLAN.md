...
Gate 1 freezes a clean source checkpoint; it does not build or admit a
repository-owned `.pak`. Generated package files, including `data.pak`, remain
outside source. In order, Foundation, all-target Workbench validation/
compilation, the five individually named source-native focused suites at JUnit
91/0/0/0, the corrected source-native force-authority canary, and source-native
Full Campaign Debug must bind to the same frozen source commit and retain their
native results.

After the gate is accepted, Workbench publishes the compiled addon directly to
the Workshop and the game downloads it from there. No bespoke local pack or
manual distribution step is part of the release plan. The external candidate
...
mission/garrison plus composition ownership delta. Formal Foundation passes at
985 references with exact source/worktree identity, zero checkout `.pak` files,
...
Run Full Campaign Debug next. Its evidence slot remains pending; no prior result
transfers. Gate 1 is in progress, release remains `NO-GO`, and Gate 2 must not
begin.
...
Gate 1 is failed and no prior result transfers. Do not begin Gate 2.
...
Gate 1 is failed and no prior result transfers.
...
Within the retired local-QA chain, CRI-103 accepted then-current-tool surface leaf
...
read-only and were jointly consumed against the unchanged package. At that
historical checkpoint, `STATUS-008` was closed. Candidate, package, gameplay,
Foundation, and Workbench facts remain unchanged. CRI-104 preserves the later
`71b276f` five-suite attempt as
timestamp-identity-rejected forensic evidence. Its raw JUnit 91/0/0/0 does not
advance the gate because two leaf prefixes disagreed with the retained start
second; the producer rejected `policy_drift` and published no aggregate or
durable receipt. If that optional historical QA chain is explicitly resumed,
all five package-bound focused suites must rerun after the one-clock correction,
followed by its corrected canary and Full Campaign Debug only after an accepted
canary. That package sequence is not current Gate 1.
...
2. Gate 1: freeze one clean source checkpoint and bind, in order, Foundation,
   ...
   force-authority canary, and source-native Full Campaign Debug to its Git
   identity.
...
5. Gate 11: cumulative canary and stable certification from the then-current
   accepted source checkpoint and its official Workbench/Workshop build, after
   rerunning every affected cumulative gate for intervening source changes.
...
## Historical Local-Candidate Ledger Migration And Activation
...
`rejected-after-corrected-canary`, and
`partisan-rc-ee0e8add2a29-20260719T063815Z` as ordered `history[2]` with
...
new immutable 5b1f candidate, appended ee0 exactly once as `history[2]`,
preserved every prior result under its original candidate, replaced the current
artifact/evidence/rung fields, and regenerated checked status without an
observable mixed state. No runtime evidence transfers. The active package later
closed `STATUS-008` with its own jointly consumed surface/retention pair at the
CRI-097 checkpoint. A subsequent bound-tool correction supersedes that pair as
active. CRI-099 accepts the fresh corrected-harness pair and closes `STATUS-008`
again. CRI-100 preserves a later rejected focused leaf, makes success-marker
parsing encoding-stable, and reopens `STATUS-008` because the bound current-
consumer bytes changed. Both passed pairs remain immutable history under their
...
certification promotion. CRI-104 preserves the next five-suite attempt as
timestamp-identity-rejected forensic evidence and requires a full five-suite
rerun after the one-clock correction; the active pair remains current and
`STATUS-008` stays closed. Gate 1 is incomplete, and release stays `NO-GO`.

## Gate 1 Frozen-Source Plan

The Gate 1 authority is a clean, frozen source commit. Run and retain, in order,
Foundation, all-target Workbench validation/compilation, every individually
named focused suite against the source-native project with aggregate JUnit
91/0/0/0, the corrected source-native force-authority canary, and source-native
...
### Retired Local-Candidate Runtime-Evidence Sequence

The following chronology records the former bespoke candidate workflow and its
package-bound QA results. Those facts remain immutable evidence for the work
that was performed, but this sequence no longer controls Gate 1 or publication.
It may be reproduced only as optional QA; current release authority comes from
the frozen source checkpoint and source-native/Workbench results above.
...
the correction before the fresh audit recorded next.

Seventh surface run
`20260722T025639Z-ee290ff3af0f46908593dbf3002050bb` completed that fresh audit
under clean harness HEAD `11a3df0`. Retail and diagnostic each passed with exact
`0 raw / 0 event` diagnostics, no crash artifacts, a complete 41-file census,
and exact cleanup with zero residue. Release-index SHA-256
`2f38ea041a7a76281b093240a7c36635f2e6bed38646f4b76254153dca4adc49`
  passed independent zero-write verification. It remains immutable passing
  evidence under its recorded tool bytes, but a later correction to the shared
  guarded-runtime module means it cannot be paired with current retention
  evidence and must be rerun under the corrected tools.

The first retention invocation failed before it created a run directory or
launched an engine. Dot-sourcing the ordinary persistence library reset the
...
to defaults. The runner now forwards all six across that import boundary, and a
64th publisher regression executes the actual boundary with distinct sentinel
  values. At that checkpoint, the correct next action was to commit the import
  correction and retry retention only against the unchanged package and recorded
  surface result.

That retry, `20260722T031531Z-434ebf5a6831`, completed four diagnostic stages.
The fifth `profile_fallback_verify` result also reported success before normal
replication shutdown and game destruction, but a process-exit timing window
between the liveness check and CIM identity recapture returned
`PGR_WAIT_IDENTITY_UNKNOWN`. The 40-process reproduction produced 27 false
unknowns before correction and zero afterward. The deterministic guarded-runtime
suite now passes 36 checks and preserves unknown for live inspection failure,
identity mismatch, or unreadable state. The owned-run failure finalizer now
deletes nothing, preserves session and permanent-NO-GO guard evidence still
present when it begins, records partial-publication state, and writes a read-only
cleanup audit plus atomic create-only failure seal. It adds no ready seal or
success output; partial `run.json` or release-index files may remain and are
recorded. Its publisher suite passes 67/67. The old unsealed failed directory
remains forensic residue and is not retrofitted. Four shutdown-stage VM
exceptions were separately classified
as three stock backend-identity diagnostics plus one stock editor disconnect-
...
Both identity corrections changed the shared tool blob bound by surface evidence,
so a fresh surface result was required at that checkpoint. Corrected run
`20260722T043428Z-6dfc9b8f53d249808d9f5f4f97516455` supplied it under clean
harness `fe018c1`: retail passed at exact `0 raw / 0 event`, diagnostic passed
with the exact approved `6 raw / 2 event` stock shutdown cluster, the 41-file
census and cleanup were exact, and release-index SHA-256
...
stages but zero standard contexts. The engine wrote three valid native saves at
`profile/.save/game`, while the snapshotter looked at `.save/game` above it and
retained only journal bytes. The first standard launch therefore reported its
requested UUID missing, started a new playthrough, and selected
`profile_fallback`. Its append-only console also never satisfied an obsolete
whole-file hash-stability rule. The failure finalizer retained the session,
sealed exact failed evidence, emitted no success controls, and left no live
...
Corrected run `20260722T070815Z-6e6d4849f5ad` then completed, published, sealed,
and independently verified all ten contexts and 251 retained files with exact
cleanup. Pair preparation caught a separate transport defect before status
changed: its index and the earlier surface index used native CRLF internally,
but Git requires LF at both tracked destinations and would change their exact
consumer-bound bytes. Both publishers now use one LF-canonical serializer for
writing and recomputation; CR-free clean-filter regressions pass in the 66/66
surface and 73/73 retention suites. Preserve both successes unchanged, commit
the corrected producers, and rerun both halves against the same package.

Focused counterattack leaf
`20260722T084353Z-03d78e016b244d8fb1aea42f307af1d6`, envelope SHA-256
`8b174b8abebb17225f3737be6934f6ec8866d4c99978106a79f94d7e8ef6102f`, retained
raw JUnit 14/0/0/0, all 14 exact success markers, the exact stamped build banner,
two approved stock diagnostics, and exact-zero cleanup. It was rejected because
the old mount parser counted six legitimate candidate/core/data `gproj` rows
instead of selecting the two candidate rows. The correction parses structured
records, requires the exact case-sensitive candidate GUID and exact guard-owned
path, admits exactly 2 records/2 exact paths/1 packed/0 invalid modes, ignores
base-project and command-line rows, binds the focused nonce path, and gives the
surface index the same structured nested attestation. Candidate, package, and
gameplay bytes are unchanged.

The corrected tool gate passes 15/15 source checks, 56 runner checks, 74 surface-
publisher checks with 73 negatives and two guarded receipts, 73/73 retention
checks, 3 valid/optional plus 53 adversarial consumer cases, and the 741-check
focused-aggregate self-test.

The next focused counterattack launch retained leaf
...
The candidate boundary was valid; mount attestation was exact at 2/2/1/0; the
process exited `0`; and JUnit 14/0/0/0, all 14 U+2705 success rows, the required
stamped build pattern and banner, two approved stock diagnostics, zero intentional
diagnostics, zero unapproved diagnostics, and exact-zero cleanup were present.
The wrapper rejected because Windows PowerShell 5.1 decoded the BOM-less UTF-8
source's literal U+2705 regex token through its legacy code page, yielding
mojibake and false `MarkerOrderExact` while the raw log character was correct.
CRI-100 replaces the
literal with ASCII source `(?:\u2705\s+)?`, constructs the real self-test marker
with `[string][char]0x2705`, and retains a wrong-prefix negative. Preserve the
leaf as rejected forensic evidence; never retrofit or promote it.

The following is an immutable passed checkpoint under its recorded tool bytes,
not the active current-consumer pair: surface leaf
`20260722T103329Z-edea9d8417884dd8a2d2b313c4543ad0` retained 41 files with
...
Both passed under harness `e2c38d2770d8ebaaa675326d1b8a91068db989e5`,
independently reverified read-only, and were jointly consumed against package
`af22d6322a215dbef466e49041fc07395cbb5ed7a5951fd3e0cee5f4a101f530`.
The `52c7e2b` pair is also immutable passed history. Neither pair satisfies the
active encoding-stable current-consumer binding, so `STATUS-008` is reopened.
Candidate, package, gameplay, Foundation, and Workbench facts are unchanged.

1. Completed under CRI-101: clean harness
   `7e2fe71d9619fd60cfe393d2776b4ed49f75186b` produced 41-file surface leaf
   `20260722T114802Z-1fb5e08619ef45b2aae20f2c5938b56c`, tracked index/ready
   SHA-256 values
   ...
   exact per-mode 2/2/1/0 mounts, retail's exact approved 6 raw/2 events,
   diagnostic's exact 0/0, and zero unapproved diagnostics. Its 251-file
   retention leaf `20260722T115003Z-4e159a24384c` has tracked index/ready
   SHA-256 values
   ...
   `standardSaveRestorationCertified: false` and `certificationClaim: none`. Both
   halves independently reverified read-only and were jointly consumed against
   unchanged package
   `af22d6322a215dbef466e49041fc07395cbb5ed7a5951fd3e0cee5f4a101f530`
   and binding
   `06b5fa23ccd10bfd1f8621f6b03c25d9c36719cc9c7d08ebcc73dabbba8c9f5f`.
   `STATUS-008` closed again without certification promotion at that checkpoint.
   Both earlier pairs remain immutable passed history, and both earlier rejected
   focused leaves remain immutable rejected history.
2. Completed under CRI-102: preserve the first `87b5ad4` five-suite attempt as
   one byte-exact superseded/rejected forensic batch. The four service suites
   passed JUnit 50/0/0/0 in total. Profile leaf
   ...
   failed closed in the wrapper despite exact package/mount identity and JUnit
   41/0/0/0. Its source-faithful diagnostic contract is exact 41/41/1: one error
   and one later non-mutating summary in each named interval, with one detailed
...
4. Completed under CRI-104: preserve the first post-pair focused attempt as
   rejected forensic batch `batch-71b276f-run-id-start-second-drift`. Clean
   harness `71b276fa9d34a1100c1ba2a27ff509a37619dbe0` and runner SHA-256
   `095ccd952e9eaaaa6f94c40053299ed5c2c8e7147ce390a29b5bd14f14da0e93`
   produced 45 files/1,365,323 bytes, JUnit 91/0/0/0, exact per-suite 2/2/1/0
   mounts, diagnostics 51 = 10 stock + 41 intentional + 0 unapproved, and zero
   cleanup/spill residue. Counterattack and QRF crossed a UTC-second boundary
   between `startedUtc` and a second clock read used for the leaf. The producer
   correctly rejected `policy_drift`, published no aggregate or durable receipt,
   and marked the batch `RED/replacement-required`. Reuse one exact
   `evidenceStartUtc` for leaf identity and `startedUtc`; keep strict consumers
   and add near-boundary plus mismatch regression coverage.
5. If this optional retired local-candidate QA sequence is explicitly resumed,
   rerun all five individually named focused profiles serially against
   ...
   No leaf from the rejected CRI-104 batch transfers.
6. Only after the focused aggregate is accepted, run the corrected canary
...
7. Run Full Campaign Debug only if that corrected-canary index is accepted.
...
mission diagnostics. That source change is sealed in historical ee0, whose
package-bound focused rung passed; it cannot repair the historical e11 package.
The historical 35/35 and 87/87 state-only result is nonpackage and did not
advance any package. Release remains `NO-GO`.
...
historical results transfers to another candidate, including active 5b1f.
...
the full profile was stopped. The fixture correction is sealed in historical ee0;
...
correction is sealed in historical ee0. Its five canonical packaged focused cases
...
5,630/5,695 with 24 unapproved diagnostics. The active 5b1f package seals the
source corrections and must prove them through its fresh ordered ladder.
...
sealed implementation/source is `7fdf3988797edeb747f5d6a6951ad0382bd93db3`, UTC
...
| Political Map | Schema 62 canonicalizes all ownership causes; Schema 63 supplies combat pressure/heat; Schema 64 supplies sole town support/population truth and contacted/territory projections. The historical focused deterministic engine case passed all three state-only groups; historical e11 canary rejected stale mission-source fixture assumptions. The correction was sealed in historical ee0, whose focused gate and corrected canary passed noncertifying at 35/35 and 87/87 before its full profile was rejected at 598/47/26/13/1 and 5,630/5,695. Former 5b and CRI-103 local-candidate results remain optional historical QA only. | From one new clean frozen source checkpoint, run the source-native 91-case focused aggregate, corrected canary, and Full Campaign Debug in order. Then prove broader native ownership consequences and classifications, real migration/restart, rendered contact/current-first/complete-territory projection, JIP/reconnect, and no bypass. Broader encounter/facility consequences, performance, and soak remain. |
