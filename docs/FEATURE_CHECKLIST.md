...
The current development save contract is the sealed Campaign Schema 65 source/Workbench checkpoint
with runtime settings still at Schema 24. It identifies implementation
...
The current development save contract is sealed at the Schema 65 source/Workbench
boundary. It retains Schema
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Versioned save state | Durable campaign facts survive restarts and schema migration. | Implemented Foundation / Sealed Schema-65 Source/Workbench Boundary | Schema 65 adds the persisted locality consequence episode/adopted-floor/last-applied envelope and exact town-event aggression before/after evidence. Schema-64 migration derives only an adopted applied floor (`0..1`) from existing current-operation/recent-fire facts, clears unavailable legacy aggression evidence, and creates/replays no consequence or strategic event. Indexed restore validates structural pool/receipt chains and exact live-town backlinks; a preset-aware post-restore pass separately quarantines aggression aimed outside configured enemy roles. Current invalid envelopes quarantine at `-65`; inconsistent town-event authority retains Schema 64's `-64` fail-close. Real serialization/restart remains open. | Highest |
...
| Full Campaign Debug | One button runs a true runtime certification suite and writes structured artifacts without changing the campaign under test. | Broad Alpha / Schema-64 Plus Sealed Schema-65 Source/Workbench Fixtures Wired / Needs Runtime Proof | Schema 65 adds casualty attribution/replay, theft, combat-episode and `HOT`-only gates, minor-locality panic-only, malformed/aggression/persistence, and pedestrian panic/recovery assertions beside the Schema-64 town suite. The state-only proof cases are complete and wired but have no native Campaign Debug artifact. They do not substitute for native entities, real restart, rendered UI, networking, reconnect, JIP, or mandatory process restart. | Highest |
...
1. Do not publish the sealed Schema-65 source/Workbench tree under the old
   Schema-64 stamp. Require every Schema-65 package and evidence artifact to
   carry implementation `609add9eeadf73816764c497178e2d35081307d1` and label
   `schema65-settings24-civilian-consequence-authority`. Complete the pending
   Foundation result and preserve every package/runtime gate below.
