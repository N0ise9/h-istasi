...
- Dynamic player markers need a marker config entry before `InsertDynamicMarker` can work.
  ...
  - HST player markers should not call `SCR_MapMarkerEntity.SetFaction()` for native stream rules. The native marker manager immediately streams faction-mismatched markers out and can locally hide hosted-server markers; keep player faction handling client-side for widget color unless per-faction marker streaming is explicitly desired.
...
- `ItemPreviewWidget` does not reliably render every inventory prefab.
  ...
  - Worn clothing Edit should first resolve the equipped item's storage node and open the storage tab for that item. Only fall back to the attachment child-node view when the equipped clothing has real inspectable attachment children; do not open an empty attachments view with only the parent row.
