...
- `AIGroup` prefab spawning is not necessarily populated on the same frame as `SpawnEntityPrefab`.
  ...
  - When spawning a bare replacement `SCR_AIGroup` for direct fallback, wrap the spawn with `SCR_AIGroup.IgnoreSpawning(true)`, immediately disable `deleteWhenEmpty`, and then attach verified faction character prefabs. This avoids empty group initialization/delete races while preserving real `AIAgent` group membership for waypoints and combat proof.
  ...
  - Direct fallback groups can report a transient zero live-count immediately after population or seating/waypoint orders. Store `m_iSpawnedAtSecond` at the actual population time and give active groups a short post-population live-count grace before survivor updates mark them eliminated; after that grace, zero live-count remains a real failure.
