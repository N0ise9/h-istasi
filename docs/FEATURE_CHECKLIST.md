...
The current sealed source checkpoint is Schema 67/settings 24. It
...
malformed authority quarantines at `-67`. It is sealed at implementation
`2798cb20b824ed74419ab6dc9bdce03f18ef71df`, UTC
`2026-07-12T23:46:02Z`, and label
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Versioned save state | Durable campaign facts survive restarts and schema migration. | Implemented Foundation / Sealed Schema-67 Source/Workbench Boundary | Schema 67 adds versioned per-enemy strategic pools and bounded mutation receipts; pre-67 migration adopts balances/aggression only, invents no history/order/spend, and current malformed authority quarantines at `-67`. Immediately preceding sealed Schema 66 continues to own exact local-security restore beneath it, followed by Schema-65/64 consequence and town-influence validation. Final Schema-67 Workbench checks pass at 5,809/11,751 with CRC `a353fa0d`; real serialization/restart remains open. | Highest |
| Runtime settings migration | Generated profile settings migrate forward without keeping obsolete setup knobs. | Implemented Foundation / Sealed Settings Schema 24 Source/Workbench | The `23 -> 24` migration adds normalized global civilian actor and traffic budgets, per-player additions, war-level budget penalty, runtime health/startup/stuck/recovery/backoff controls, and preserves explicit older values. Campaign persistence independently advances to the sealed Schema-67 source checkpoint; generated settings remain Schema 24. Generated profile rewrite and packaged migration proof are pending. | Keep |
