...
| Generated routes | Convoys, QRFs, patrols, roadblocks, and mission movement use route-aware paths. | Broad Alpha | Support/QRF deployments now use generated route chains and assign AI waypoints to routed infantry; extend this into richer garrison behavior and specialized waypoint types. | Highest |
...
| Physical response | Near-player enemy responses spawn, move, fight, and fold back. | Broad Alpha / Needs Soak | QRF/support infantry now receives generated-route AI waypoints; next add richer HQ-pressure/counterattack behavior and true mixed vehicle spawning. | Highest |
...
| Feature | Target behavior | Current status | Gap / next work | Priority |
| --- | --- | --- | --- | --- |
| Garrison state | Captured zones store abstract defender manpower and vehicles. | Implemented Foundation | Add player-facing management polish and costs. | High |
| Recruitment and HR costs | Commander spends HR/money to recruit forces. | Broad Alpha | Add arsenal/equipment requirements and clearer UI. | High |
...
| Mission runtime primitives | HVT, destroy, hold/clear, rescue, cargo, convoy, and support primitives have physical action paths. | Broad Alpha | Replace MVP primitives with mission-specific props, interactions, and consequences. | Highest |
...
| Vehicle capture and garage | Vehicles can be captured, stored, redeployed, and restored with cargo/heat metadata. | Broad Alpha / Needs Soak | Add capacity, class limits, fuel/damage polish, and repeated restart tests. | High |
...
| Map markers | HQ, zones, missions, support, QRFs, and orders publish linked markers with cleanup proof. | Broad Alpha | Continue owner-client visual proof and marker/backing consistency checks. | High |
| Full Campaign Debug | One button runs a true runtime certification suite and writes structured artifacts. | Broad Alpha | Keep adding ARRANGE/ACT/OBSERVE/ASSERT/CLEANUP cases for every new feature. | Highest |
