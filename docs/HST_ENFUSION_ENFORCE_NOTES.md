...
- Loadout editor saved loadouts:
  ...
  - To test physical saved-loadout reflection, seed a second transient non-serialized loadout with explicit finite slots, apply it through `RequestMemberApplySavedLoadout()`, count the player inventory before/after, then apply a transient restore loadout that uses the original native serialized string. Keep this debug-only and restore arsenal/issued state afterward; rendered editor UI and exact equipment-slot visual proof remain separate.
  - Physical saved-loadout probes must arrange enough carried storage for their finite inventory items. A full-profile checkpoint run showed that a magazine-only non-serialized loadout can fail the real apply path with `Inventory Full` when the current player outfit has no suitable free storage. Seed a transient backpack/carrier slot before the finite test magazines, preflight the carrier prefab, and restore/remove its arsenal record during cleanup.
...
- Convoy mission runtime should not fall back to a generic mission prop when convoy vehicle asset planning fails.
  ...
  - Convoy debug readiness assertions should respect `m_bPendingGrace`: missing seated drivers, routes, or waypoints during the staging grace are WARN evidence for async crew boarding, not hard failures. After the grace expires, the same missing readiness fields are real convoy failures.
