...
- Convoy mission runtime should not fall back to a generic mission prop when convoy vehicle asset planning fails.
  ...
  - Convoy setup cannot rely only on `SCR_CompartmentAccessComponent.MoveInVehicle()` owner-RPC boarding. Dedicated/server-side debug convoys need a bounded server-authoritative `GetInVehicle(slotOwner, slot, true, ...)` path for spawned AI crew, then an immediate compartment rescan before judging driver availability. Otherwise direct fallback crew can exist while readiness stays stuck at `waiting for animated AI boarding to seat a driver`.
