...
- Campaign debug certification output should be structured first, text second.
  ...
  - When a phase smoke step has a typed case builder, keep its command/report string as recent-log or case evidence only. Do not also emit a duplicate legacy pass/fail case for the same action; duplicate rows inflate the summary matrix and bypass the typed cleanup model. Current Phase 14-24 smoke uses typed cases for indices 3-62, including Phase 20/21 report steps.
...
- Player-requested support cooldowns can poison later support smoke steps.
  ...
  - Phase 23 UI/marker smoke should assert command/menu and marker state directly: no `missing visible command:` or `missing dispatch:` detail rows, admin menu strings for campaign-debug and Phase-23 controls, every `hst_zone_<zoneId>` marker model entry with linked ID, owner, color, style, and position matching the current `HST_ZoneState`, HQ/mission/support/QRF marker coverage from `HST_CampaignState.m_aMapMarkers`, and marker/backing-state consistency via the same helpers used by post-case cleanup. Failed-action samples should snapshot state before and after invalid commands so they prove reasoned failure with no campaign mutation. Native map-marker manager absence should be explicit WARN/report evidence; visual widget inspection still requires a separate UI/render probe.

  - Campaign-debug read-only report snapshots should use typed `observation.*` cases instead of the `legacy.*` text wrapper. The observation case should record the full report as evidence, assert non-empty output, and run the normal failure-string classifier as an assertion so baseline/final reports appear in matrices without claiming feature-specific physical coverage.
