...
- Full-campaign debug coverage should explicitly map to the phase plan instead of assuming late smoke helpers cover everything.
  ...
  - Phase 12 persistence smoke should seed sentinel state first, then record typed assertions against the actual `HST_CampaignState`: expected-summary task, active smoke missions, convoy and primitive mission matrices, mission assets/runtime entities, garage cargo, support/order sentinels, civilian records, and undercover records. Use `HST_CampaignSaveData.Capture()` plus `Restore()` for an in-memory roundtrip and compare the smoke summary/report/counts between live and restored state. The immediate smoke/report commands may return `WARN` if live systems mutate counts between seed and report, but that is drift evidence only when `missing/zero none` remains true; the live-vs-restored summary/count assertions are the hard persistence-fidelity proof.
...
- `AIGroup` prefab spawning is not necessarily populated on the same frame as `SpawnEntityPrefab`.
  ...
  - If an `AIGroup` runtime root exists for a group id, live-count helpers should treat that native group as authoritative and should not fall back to direct tracked entity counts. Direct entity counting is only a legacy/direct-only fallback when no AIGroup root exists.
  ...
  - When direct fallback members are tracked beside their parent `AIGroup`, counting must not double-count or mask a broken native group. Count living controlled `AIGroup.GetAgents()` members as the source of truth while an AIGroup root exists; only count tracked member entities directly when there is no AIGroup root at all.
